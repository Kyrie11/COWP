...
PROBE="$OUT_ROOT/eval/probe/delta_conventional_vs_root_transport.json"
"$PYTHON_BIN" - "$MECH" "$PROBE" <<'PY'
import json,sys,os
x=json.load(open(sys.argv[1],encoding='utf-8'))
assert x.get('pass') is True and x.get('calibration_feasible') is True
assert x.get('heldout_certificate_semantics_current') is True
assert x.get('calibration_certificate_semantics_current') is True
assert os.path.isfile(sys.argv[2]), f'run/inspect probe first: {sys.argv[2]}'
PY
...
for k in ['natural_checkpoint','natural_history','transport_checkpoint','planner_checkpoint']: print(x[k]['path'])
PY
)
  export NATURAL_CKPT="${NATURAL_CKPT:-${PATHS[0]}}" NATURAL_HISTORY="${NATURAL_HISTORY:-${PATHS[1]}}"
  export TRANSPORT_CKPT="${TRANSPORT_CKPT:-${PATHS[2]}}" CKPT="${CKPT:-${PATHS[3]}}"
elif [[ -s "$TRANSFER" ]]; then
  mapfile -t PATHS < <("$PYTHON_BIN" - "$TRANSFER" <<'PY'
import json,sys
x=json.load(open(sys.argv[1],encoding='utf-8')); print(x['natural_checkpoint']); print(x['natural_history'])
PY
)
  export NATURAL_CKPT="${NATURAL_CKPT:-${PATHS[0]}}" NATURAL_HISTORY="${NATURAL_HISTORY:-${PATHS[1]}}"
...
for p in "$NATURAL_CKPT" "$NATURAL_HISTORY" "$TRANSPORT_CKPT" "$CKPT"; do [[ -s "$p" ]] || { echo "missing artifact: $p" >&2; exit 2; }; done
export RUN_DIAGNOSE=0 RUN_NATURAL=0 RUN_TRANSPORT=0 RUN_PLANNER=0 RUN_OFFLINE=0
export RUN_PROBE=0 RUN_FULL=1 FORCE_TRAIN=0 FORCE_EVAL="${FORCE_EVAL:-1}"
export STOP_AFTER_STAGE=none REQUIRE_INIT_CKPT=0 REQUIRE_WAYMAX_PREFLIGHT=1
