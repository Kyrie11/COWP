...
CONFIG_CANDIDATE_DIR="$(mktemp -d "$OUT_ROOT/configs/.candidate_v16_1.XXXXXX")"
...
"$PYTHON_BIN" -u -m cowp.scripts.42_write_run_provenance \
  ...
  --file "run_cowp_v16_1_dual_gpu.sh=$0" | tee "$OUT_ROOT/logs/run_provenance.log"
...
if [[ "$DETACH" == "1" && "${COWP_V16_1_DETACHED:-0}" != "1" ]]; then
  export COWP_V16_1_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v16.1] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
if [[ "$RUN_DIAGNOSE" == "1" ]]; then
  ...
  for pid in "${diag_pids[@]:-}"; do [[ -n "$pid" ]] && wait "$pid"; done
...
if [[ "$STOP_AFTER_STAGE" == "diagnose" ]]; then
  echo "[cowp_v16.1] stopped after diagnostics: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "natural" ]]; then
  echo "[cowp_v16.1] stopped after natural gate: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "transport" ]]; then
  echo "[cowp_v16.1] stopped after transport: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "planner" ]]; then
  echo "[cowp_v16.1] stopped after planner: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "offline" ]]; then
  echo "[cowp_v16.1] stopped after learned-offline verification: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "probe" ]]; then
  echo "[cowp_v16.1] stopped after probe: $OUT_ROOT"
  exit 0
fi
