...
# v16.8.35: Waymax-Aligned Control-Projected Recovery Option Spectrum
# Main probe: Control-Projected Option-Spectrum Hysteresis (CPOSH)
# Diagnostic: Waymax-Kinematic-Guarded ROSH (WK-ROSH)
#
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_35_control_projected_option_spectrum}"
...
REF_CF48_V34_TG="$SCRIPT_DIR/reference_results/v16_8_34_counterfactual48_tg_rosh_reference.json"
REF_CF48_V34_EOSH="$SCRIPT_DIR/reference_results/v16_8_34_counterfactual48_eosh_reference.json"
...
map_key={
    'cowp_waymax_kinematic_guarded_rosh':'waymax_kinematic_guarded',
    'cowp_control_projected_option_spectrum_hysteresis':'control_projected_spectrum',
}
gates=d.get('preregistered_gate',{})
for method in [x.strip() for x in methods.split(',') if x.strip()]:
    key=map_key.get(method)
    if key is None:
        raise SystemExit(f'ERROR: {method} is not a v16.8.35 promotion candidate')
    g=gates.get(key)
...
analyze_new_set() {
  ...
  local args=(--cowp "$cowp" --rvr "$rvr" --stage "$stage" --development-selected \
    --output "$OUT_ROOT/${tag}_control_projected_option_spectrum_analysis.json")
  if [[ "$stage" == "counterfactual48" ]]; then
    args+=(--v33-rosh "$REF_CF48_V33_ROSH" \
      --v34-transition-guarded "$REF_CF48_V34_TG" \
      --v34-executable-spectrum "$REF_CF48_V34_EOSH")
  fi
  [[ -s "$OUT_ROOT/${tag}_cowp_waymax_kinematic_guarded_rosh_merged.json" ]] && \
    args+=(--waymax-kinematic-guarded "$OUT_ROOT/${tag}_cowp_waymax_kinematic_guarded_rosh_merged.json")
  [[ -s "$OUT_ROOT/${tag}_cowp_control_projected_option_spectrum_hysteresis_merged.json" ]] && \
    args+=(--control-projected-spectrum "$OUT_ROOT/${tag}_cowp_control_projected_option_spectrum_hysteresis_merged.json")
  python -m cowp.scripts.88_analyze_control_projected_option_spectrum "${args[@]}"
}
...
case "$MODE" in
  sanity)
    python -m py_compile \
      ...
      cowp/scripts/87_analyze_executable_option_spectrum.py cowp/scripts/88_analyze_control_projected_option_spectrum.py
    python -m pytest -q \
    ...
    ensure_ids
    echo "v16.8.35 focused semantic/integrity sanity passed"
    ;;
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v35" "$CF48_IDS" \
      "${METHODS:-cowp_waymax_kinematic_guarded_rosh,cowp_control_projected_option_spectrum_hysteresis}"
    ;;

  analyze_counterfactual48)
    analyze_new_set "counterfactual48" "counterfactual48_v35" "$REF_CF48_COWP" "$REF_CF48_RVR"
    ;;

  fresh37_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_control_projected_option_spectrum_hysteresis}"
    require_gate_pass "$OUT_ROOT/counterfactual48_v35_control_projected_option_spectrum_analysis.json" "$promoted" "counterfactual48"
    run_parallel_methods "fresh37_v35" "$FRESH37_IDS" "$promoted"
    ;;

  analyze_fresh37)
    analyze_new_set "fresh37" "fresh37_v35" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR"
    ;;

  confirm200_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_control_projected_option_spectrum_hysteresis}"
    require_gate_pass "$OUT_ROOT/fresh37_v35_control_projected_option_spectrum_analysis.json" "$promoted" "fresh37"
  ...
  analyze_confirm200)
    local_args=(--cowp "$REF_EXACT_COWP" --rvr "$REF_EXACT_RVR" --stage exact200 \
      --output "$OUT_ROOT/confirm_exact200_control_projected_option_spectrum_analysis.json")
    [[ -s "$OUT_ROOT/confirm_exact200_cowp_waymax_kinematic_guarded_rosh_merged.json" ]] && \
      local_args+=(--waymax-kinematic-guarded "$OUT_ROOT/confirm_exact200_cowp_waymax_kinematic_guarded_rosh_merged.json")
    [[ -s "$OUT_ROOT/confirm_exact200_cowp_control_projected_option_spectrum_hysteresis_merged.json" ]] && \
      local_args+=(--control-projected-spectrum "$OUT_ROOT/confirm_exact200_cowp_control_projected_option_spectrum_hysteresis_merged.json")
    python -m cowp.scripts.88_analyze_control_projected_option_spectrum "${local_args[@]}"
    ;;

  help|*)
    cat <<EOF
v16.8.35 recommended order
...
3) Mechanism-selected 48-scene panel; run BOTH causal hypotheses:
   ...
   WK-ROSH = v16.8.33 semantic spectrum plus a hard guard aligned to Waymax's evaluated inverse acceleration/steering-curvature metric.
   CPOSH   = WK-ROSH plus a successor spectrum built from candidate trajectories projected through the actual stateful COWP controller.

4) ONLY method(s) whose JSON preregistered_gate.pass == true:
   PROMOTED_METHODS=cowp_control_projected_option_spectrum_hysteresis $0 fresh37_parallel2
   $0 analyze_fresh37

   fresh37 is disjoint from ALL v16.8.30 panels and the v16.8.31 holdout64.
   It was selected by ID only, not by v16.8.35 outcomes.  It is still development evidence.
