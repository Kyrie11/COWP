...
# Paper-grade data rebuild for v16.8.3 RMR-BCTE. Uses a fresh root because priority labels,
...
COWP_ROOT="${COWP_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_3_rmr_bcte}"
...
TRANSPORT_TRAIN="$COWP_ROOT/tensor_cache_train_waymax_transport_v16_8_3"
TRANSPORT_VAL="$COWP_ROOT/tensor_cache_val_waymax_transport_v16_8_3"
REPLAY_DIR="$COWP_ROOT/waymax_replay_v16_8_3"
...
if [[ -s "$FINGERPRINT_FILE" ]]; then
  ...
  echo "existing labels without a v16.8.3 fingerprint; use a new COWP_ROOT or set ALLOW_UNVERIFIED_RESUME=1 after manual audit" >&2
  exit 3
else
...
run augment_train "$PYTHON_BIN" -u -m cowp.scripts.26_augment_transport_labels \
  ...
  --sidecar-subdir .transport_v16_8_3 --force
...
cat > "$COWP_ROOT/data_manifest_v16_8_3.json" <<JSON
{
  "schema_version": "cowp_v16_8_3_rmr_bcte_data_v1",
  ...
  "notes": "Fresh labels include jerk-consistent, provenance-tracked RMR-BCTE interaction-timing proposals, explicit root-conditioned response search, transported OPR, hard protected-relation semantics, and map-screened proposals."
}
JSON

echo "[v16.8.3 RMR-BCTE data] complete: $COWP_ROOT"
