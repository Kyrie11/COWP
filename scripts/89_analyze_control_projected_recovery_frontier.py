...
def _bootstrap_mean_ci(x: np.ndarray, seed: int = 16836, draws: int = 10000) -> list[float] | None:
...
def _aggregate(rows: dict[str, dict[str, Any]], ids: list[str]) -> dict[str, Any]:
    ...
    diag_keys = [
        ...
        "control_projected_recovery_frontier_step_rate", "recovery_frontier_probe_step_rate",
        "mean_recovery_frontier_representative_count_on_probes", "mean_recovery_frontier_profiles_evaluated_on_probes",
        "mean_recovery_frontier_strict_admissible_count_on_probes", "mean_recovery_frontier_weak_admissible_count_on_probes",
        "mean_recovery_frontier_current_prefix_admissible_count_on_probes",
        "recovery_frontier_selected_non_rvr_rate_on_switches", "recovery_frontier_selected_historical_rvr_rate_on_switches",
        "mean_recovery_frontier_selected_fallback_score_delta_on_switches", "mean_recovery_frontier_selected_prefix_delta_on_switches",
...
def main() -> None:
    ...
    ap.add_argument("--control-projected-recovery-frontier", required=True)
    ...
    data = {
        ...
        "control_projected_recovery_frontier": _load(args.control_projected_recovery_frontier),
    }
    ...
    out: dict[str, Any] = {
        "schema_version": "cowp_control_projected_recovery_frontier_analysis_v1", "development_selected": bool(args.development_selected),
    ...
    name = "control_projected_recovery_frontier"
    if "rvr" in rowmap:
    ...
    # Mechanism diagnostic, deliberately not part of the inherited promotion gate.
    out["frontier_mechanism_diagnostic"] = {
        "selected_non_historical_rvr_rate_on_switches": out["methods"][name].get("recovery_frontier_selected_non_rvr_rate_on_switches"),
        "mean_representative_count_on_probes": out["methods"][name].get("mean_recovery_frontier_representative_count_on_probes"),
        "mean_strict_admissible_count_on_probes": out["methods"][name].get("mean_recovery_frontier_strict_admissible_count_on_probes"),
        "interpretation": "If non-historical-RVR selection remains zero, the existing-bank frontier did not expand effective recovery support and should not be claimed as the mechanism even if outcomes fluctuate.",
    }
