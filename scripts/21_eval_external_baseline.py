...
def learned_offline_eval(args: argparse.Namespace, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(iterator, start=1):
            ext = make_external_batch(batch, model_cfg, device=device, max_neighbors=max_neighbors, max_candidates=max_candidates, horizon=future_len, baseline=baseline, require_candidates=True, require_future=True)
            if baseline == "gameformer":
                ...
                raise ValueError(f"Unsupported external baseline for learned_offline eval: {baseline}")
...
def waymax_eval(args: argparse.Namespace, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    policy = make_external_waymax_policy(
        ...
        profile_policy_runtime=bool(getattr(args, "profile_policy_runtime", False)),
        profile_policy_runtime_sync=bool(getattr(args, "profile_policy_sync", False)),
    )
    ...
    _log(
        ...
        f"shard={args.shard_index}/{args.num_shards} horizon={horizon} progress={not args.no_progress} "
        f"standard_metrics={args.waymax_standard_metrics} metric_names={metric_names if metric_names is not None else 'default/all'}"
    )
    t0 = time.time()
    ...
    metric_names_payload = sorted(metric_names) if isinstance(metric_names, set) else args.waymax_standard_metric_names
    ...
    payload: dict[str, Any] = {
        ...
        "waymax_standard_metric_names": metric_names_payload,
        ...
        "profile_policy_runtime": bool(getattr(args, "profile_policy_runtime", False)),
        "profile_policy_sync": bool(getattr(args, "profile_policy_sync", False)),
    }
...
def main() -> None:
    ...
    ap.add_argument("--execution-mode", choices=["auto", "direct", "candidate_tree"], default="auto")
    ...
    ap.add_argument("--profile-policy-runtime", action="store_true")
    ap.add_argument("--profile-policy-timing", dest="profile_policy_runtime", action="store_true")
    ap.add_argument("--profile-policy-sync", action="store_true")
