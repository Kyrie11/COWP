...
def _aggregate(rows: dict[str, dict[str, Any]], ids: list[str]) -> dict[str, Any]:
    ...
    diag_keys = [
        ...
        "conflict_window_control_reachable_tube_step_rate",
...
def main() -> None:
    ...
    parser.add_argument("--conflict-window-control-reachable-tube", required=True)
    ...
    data = {
        ...
        "conflict_window_control_reachable_tube": _load(
            args.conflict_window_control_reachable_tube
        ),
    }
    ...
    out: dict[str, Any] = {
        "schema_version": "cowp_conflict_window_control_reachable_tube_analysis_v1",
    ...
    name = "conflict_window_control_reachable_tube"
    if "rvr" in rowmap:
    ...
    out["conflict_window_control_tube_mechanism_diagnostic"] = {
        **pooled,
        "event_release_only_support_is_attribution_not_gate": True,
        "interpretation": (
            "The inherited six-item outcome gate is unchanged. Event-release-only "
            "support and event-release selection are required to attribute any gain "
            "specifically to the V39 control-sequence expansion, but are not post-hoc "
            "additional promotion thresholds."
        ),
    }
