...
def _bootstrap_mean_ci(x: np.ndarray, seed: int = 16831, draws: int = 10000) -> list[float] | None:
...
def main() -> None:
    ...
    out: dict[str, Any] = {
        "schema_version": "cowp_bihorizon_option_viability_analysis_v1",
    ...
    if "rvr" in rowmap:
        ...
        for name in ("sov", "bihorizon", "restore"):
    ...
    if "sov" in rowmap:
        out["paired_vs_sov"] = {}
        for name in ("bihorizon", "restore"):
            if name in rowmap:
                out["paired_vs_sov"][name] = _paired(rowmap["sov"], rowmap[name], ids)
