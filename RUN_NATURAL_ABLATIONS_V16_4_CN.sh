...
MAIN_OUT_ROOT="${MAIN_OUT_ROOT:-outputs/cowp_v16_4_natural_recovery_v9labels_seed2026}"
ABL_ROOT="${ABL_ROOT:-outputs/cowp_v16_4_natural_ablations_v9labels_seed2026}"
...
[[ -s "$MAIN_GATE" ]] || { echo "missing main effectiveness gate: $MAIN_GATE" >&2; exit 2; }
"$PYTHON_BIN" - "$MAIN_GATE" <<'PY'
import json, sys
p = sys.argv[1]
x = json.load(open(p, encoding="utf-8"))
assert bool(x.get("pass", False)), f"main natural effectiveness gate failed: {p}"
PY
...
run_ablation() {
  ...
  if [[ "${FORCE_TRAIN:-0}" == "1" || ! -s "$out/checkpoints/cowp_natural_best.pt" || ! -s "$out/checkpoints/history_natural.json" ]]; then
    env CUDA_VISIBLE_DEVICES="$TRAIN_VISIBLE_DEVICES" \
      ...
      --natural-graph-warmup-epochs 2 --grad-clip 1.0 --amp-dtype auto --fused-adamw \
...
# Same loss and OBS capacity, but remove the integrated trajectory trust region.
run_ablation no_integrated_trust_region configs/model_cowp_v16_no_trust_region.yaml configs/train_cowp_v16.yaml
...
"$PYTHON_BIN" -u -m cowp.scripts.41_compare_natural_ablations \
  ...
  --no-trust-region "$ABL_ROOT/no_integrated_trust_region/eval/learned_natural_effectiveness.json" \
