...
# v16.8.3: 先测 proposal ceiling，再决定是否支付约 4 天的完整重建成本。
...
PROBE_ROOT="${PROBE_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_3_proposal_probe}"
...
FRESH_LABELS="$PROBE_ROOT/labels_val_rmr_bcte"
...
#    1) 修正后的 jerk filter；2) RMR-BCTE；3) proposal provenance。
...
print("\n===== v16.8.3 proposal probe verdict =====")
print(json.dumps({
...
if p["promote_to_full_rebuild"]:
    print("PASS: 允许进入 PREPARE_COWP_V16_8_3_DATA_CN.sh 全量重建。")
else:
