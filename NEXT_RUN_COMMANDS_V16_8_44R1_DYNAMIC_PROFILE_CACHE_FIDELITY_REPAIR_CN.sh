...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_44r1_dynamic_profile_cache_fidelity_repair}"
...
run_shard(){ local gpu="$1" shard="$2" ids="$3" out="$4";
  CUDA_VISIBLE_DEVICES="$gpu" XLA_PYTHON_CLIENT_PREALLOCATE=false python -m cowp.scripts.04_eval_closed_loop "${common[@]}" "${index_arg[@]}" \
    ...
    --num-shards 2 --shard-index "$shard" --profile-policy-runtime --no-progress --status-every 1 --output "$out"; }
...
case "$MODE" in
  sanity)
    ...
    echo "V16.8.44R1 dynamic-profile cache-fidelity sanity PASS" ;;
...
# R1 is engineering-only: scientific method/gates remain V16.8.44; old lost7 outputs must not be reused." ;;
esac
