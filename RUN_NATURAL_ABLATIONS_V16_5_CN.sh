...
ABL_ROOT="${ABL_ROOT:-outputs/cowp_v16_5_natural_ablations_v9labels_seed2026}"
...
[[ -s "$MAIN_REPORT" && -s "$MAIN_GATE" ]] || { echo "run strict v16.5 natural recovery first" >&2; exit 2; }
...
run_ablation() {
  ...
  if [[ "${FORCE_TRAIN:-0}" == "1" || ! -s "$out/checkpoints/cowp_natural_best.pt" ]]; then
    env CUDA_VISIBLE_DEVICES="$TRAIN_VISIBLE_DEVICES" \
      ...
      --val-every "$VAL_EVERY" --device cuda --output-dir "$out/checkpoints" \
      --early-stop-patience 8 --early-stop-min-delta 1e-4 \
...
# One-factor-at-a-time controls from the same checkpoint, seed, data and schedule.
run_ablation no_obs_capacity_boost configs/model_cowp_v16_no_obs_capacity.yaml configs/train_cowp_v16.yaml
run_ablation no_mass_aware_root_envelope configs/model_cowp_v16.yaml configs/train_cowp_v16_no_mass_trust.yaml
...
"$PYTHON_BIN" -u -m cowp.scripts.41_compare_natural_ablations \
  ...
  --no-mass-trust "$ABL_ROOT/no_mass_aware_root_envelope/eval/learned_natural_effectiveness.json" \
