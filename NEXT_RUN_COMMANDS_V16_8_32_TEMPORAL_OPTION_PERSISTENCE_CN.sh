...
# v16.8.32: Temporal Option Persistence (TOP)
# Main probe: Tri-Horizon Option Persistence (THOP)
# Diagnostic: SOV-triggered Recovery Commitment (SRC)
#
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_32_temporal_option_persistence}"
...
analyze_new_set() {
  ...
  local args=(--cowp "$cowp" --rvr "$rvr" --stage "$stage" --development-selected \
    --output "$OUT_ROOT/${tag}_temporal_option_analysis.json")
  if [[ "$stage" == "counterfactual48" ]]; then
    args+=(--sov "$REF_CF48_SOV" --bihorizon "$REF_CF48_BHOV")
  fi
  [[ -s "$OUT_ROOT/${tag}_cowp_trihorizon_option_persistence_merged.json" ]] && \
    args+=(--trihorizon "$OUT_ROOT/${tag}_cowp_trihorizon_option_persistence_merged.json")
  [[ -s "$OUT_ROOT/${tag}_cowp_sov_recovery_commitment_merged.json" ]] && \
    args+=(--commitment "$OUT_ROOT/${tag}_cowp_sov_recovery_commitment_merged.json")
  python -m cowp.scripts.85_analyze_temporal_option_persistence "${args[@]}"
}
...
case "$MODE" in
  sanity)
    python -m py_compile \
      ...
      cowp/scripts/82_verify_cowp_equivalence.py cowp/scripts/85_analyze_temporal_option_persistence.py
    python -m pytest -q \
    ...
    ensure_ids
    echo "v16.8.32 focused semantic/integrity sanity passed"
    ;;
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v32" "$CF48_IDS" \
      "${METHODS:-cowp_trihorizon_option_persistence,cowp_sov_recovery_commitment}"
    ;;

  analyze_counterfactual48)
    analyze_new_set "counterfactual48" "counterfactual48_v32" "$REF_CF48_COWP" "$REF_CF48_RVR"
    ;;

  fresh37_parallel2)
    ensure_ids
    # Run ONLY methods that passed the counterfactual48 preregistered development gate.
    run_parallel_methods "fresh37_v32" "$FRESH37_IDS" \
      "${PROMOTED_METHODS:-cowp_trihorizon_option_persistence}"
    ;;

  analyze_fresh37)
    analyze_new_set "fresh37" "fresh37_v32" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR"
    ;;

  confirm200_parallel2)
    ensure_ids
    # Development confirmation only.  Never rerun COWP/RVR: immutable exact200
    # references are bundled.  Use only methods that passed fresh37.
    run_parallel_methods "confirm_exact200" "$EXACT_IDS" \
      "${PROMOTED_METHODS:-cowp_trihorizon_option_persistence}"
    ;;

  analyze_confirm200)
    local_args=(--cowp "$REF_EXACT_COWP" --rvr "$REF_EXACT_RVR" --stage exact200 \
      --output "$OUT_ROOT/confirm_exact200_temporal_option_analysis.json")
    [[ -s "$OUT_ROOT/confirm_exact200_cowp_trihorizon_option_persistence_merged.json" ]] && \
      local_args+=(--trihorizon "$OUT_ROOT/confirm_exact200_cowp_trihorizon_option_persistence_merged.json")
    [[ -s "$OUT_ROOT/confirm_exact200_cowp_sov_recovery_commitment_merged.json" ]] && \
      local_args+=(--commitment "$OUT_ROOT/confirm_exact200_cowp_sov_recovery_commitment_merged.json")
    python -m cowp.scripts.85_analyze_temporal_option_persistence "${local_args[@]}"
    ;;

  help|*)
    cat <<EOF
v16.8.32 recommended order
...
3) Mechanism-selected 48-scene panel; run BOTH causal hypotheses:
   ...
   THOP = tests one-step-myopia via a second emitted-action option-persistence guard.
   SRC  = tests hybrid-mode/chattering via strict-SOV entry + state-restoration exit.

4) ONLY method(s) whose JSON preregistered_gate.pass == true:
   PROMOTED_METHODS=cowp_trihorizon_option_persistence $0 fresh37_parallel2
   $0 analyze_fresh37

   fresh37 is disjoint from ALL v16.8.30 panels and the v16.8.31 holdout64.
   It was selected by ID only, not by v16.8.32 outcomes.  It is still development evidence.
