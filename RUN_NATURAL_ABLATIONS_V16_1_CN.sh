...
MAIN_OUT_ROOT="${MAIN_OUT_ROOT:-outputs/cowp_v16_1_cnob_dynamics_v9labels_seed2026}"
ABL_ROOT="${ABL_ROOT:-outputs/cowp_v16_1_natural_ablations_v9labels_seed2026}"
...
# 默认后台运行整套消融；前台调试设置 BACKGROUND=0。
export BACKGROUND="${BACKGROUND:-1}"
if [[ "$BACKGROUND" == "1" && "${COWP_V16_1_ABL_BACKGROUND_CHILD:-0}" != "1" ]]; then
  export COWP_V16_1_ABL_BACKGROUND_CHILD=1
  mkdir -p "$ABL_ROOT/logs"
  nohup env BACKGROUND=0 bash "$0" > "$ABL_ROOT/logs/driver.nohup.log" 2>&1 < /dev/null &
  pid=$!
  printf '%s\n' "$pid" > "$ABL_ROOT/logs/driver.pid"
  echo "[cowp_v16.1 ablations] background pid=$pid"
  echo "[cowp_v16.1 ablations] log=$ABL_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
run_ablation() {
  ...
  if [[ "${FORCE_TRAIN:-0}" == "1" || ! -s "$out/checkpoints/cowp_natural_best.pt" || ! -s "$out/checkpoints/history_natural.json" ]]; then
    env CUDA_VISIBLE_DEVICES="$TRAIN_VISIBLE_DEVICES" \
      ...
      --num-workers "$WORKERS" --prefetch-factor "$PREFETCH" --device cuda \
