...
# v16.8.5 engineering-optimized paper-grade rebuild for the unchanged v16.8.4 BCS-RMR-BCTE method.
...
COWP_ROOT="${COWP_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_5_bcs_rmr_bcte}"
...
TRANSPORT_TRAIN="$COWP_ROOT/tensor_cache_train_transport_v16_8_4"
TRANSPORT_VAL="$COWP_ROOT/tensor_cache_val_transport_v16_8_4"
REPLAY_DIR="$COWP_ROOT/waymax_replay_v16_8_4"
...
if [[ -s "$FINGERPRINT_FILE" ]]; then
  ...
  echo "existing labels without a v16.8.4 fingerprint; use a new COWP_ROOT or set ALLOW_UNVERIFIED_RESUME=1 after manual audit" >&2
  exit 3
else
...
run augment_train "$PYTHON_BIN" -u -m cowp.scripts.26_augment_transport_labels \
  ...
  --sidecar-subdir .transport_v16_8_4 --force
...
cat > "$COWP_ROOT/data_manifest_v16_8_4.json" <<JSON
{
  "schema_version": "cowp_v16_8_4_bcs_rmr_bcte_data_v1",
  ...
  "notes": "Fresh labels include jerk-consistent, boundary-entry-time-consistent BCS-RMR-BCTE timing proposals with causal agent arrival envelopes and realized-TTA validation; explicit root-conditioned response search, transported OPR, hard protected-relation semantics, and map-screened proposals."
}
JSON

echo "[v16.8.4 Boundary-Consistent Smooth RMR-BCTE (BCS-RMR-BCTE) data] complete: $COWP_ROOT"
