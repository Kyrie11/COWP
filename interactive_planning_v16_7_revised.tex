...
\end{equation}
where the small mass floor $\varepsilon_p$ prevents a learned predictor from hiding a safety-relevant root by assigning it zero probability. A candidate is priority-aware non-coercively feasible only if every $i\in\mathcal{P}^k$ has at least one same-root low-burden safe response and $O_i(\tau^k_e)\ge\alpha$. The corresponding all-critical condition is reported as a global stress diagnostic rather than used as the primary hard veto. This finite, root-indexed definition replaces an ill-defined set intersection between continuous trajectory samples and matches the implemented transport labels.
...
\end{equation}
The implementation does not solve an unrestricted continuous optimal-control problem for every candidate. Instead, it constructs a typed, comfort-bounded response bank $\mathcal{Q}_i(\tau_e^k)$ and retains the lowest-burden safe members with a finite beam search:
\begin{equation}
    \mathcal{R}^{\mathrm{opt}}_i(\tau_e^k)
    =\operatorname{TopB}_{\tau_i\in\mathcal{Q}_i(\tau_e^k)}
    \left\{B_i^{\mathrm{prim}}(\tau_i;\mathcal{S},\tau_e^k):
    \mathrm{Collide}=0,\ \mathrm{DynOK}=1,\ \mathrm{MapOK}=1\right\}.
\end{equation}
The bank varies comfort deceleration, reaction delay, recovery acceleration, and limited lateral slack. Emergency hard-braking or evasive-stop primitives are kept as a separate source and are used to determine whether collision freedom exists only at high burden; their mere existence does not certify non-coercive feasibility.
