...
We answer this question by introducing \emph{non-coercive feasibility}. A candidate ego trajectory is non-coercively feasible if every critical road user retains at least one low-burden safe response and, more importantly, if the ego does not collapse that road user's natural action space into a narrow or high-burden escape maneuver. This changes the role of social interaction in planning. Instead of adding another courtesy-like soft penalty, we treat safety-by-coercion as a feasibility defect. A plan that relies on another vehicle's hard braking, abrupt yielding, priority abandonment, or gap surrender is not merely less polite; it is false-safe. 
...
\item We formulate \emph{non-coercive feasibility}, a planning-time criterion requiring critical agents to retain sufficiently rich low-burden safe response sets, rather than being forced into narrow or costly evasive behavior. 
...
\end{equation}
where $\tau^k_e$ is a continuous ego trajectory and $a^k_e$ is its macro intent. We use a graph-conditioned lattice-MPC proposal module: macro intents determine topological maneuver classes, while continuous trajectories are generated by solving short-horizon kinodynamic optimal control problems under route, lane, acceleration, jerk, and curvature constraints. 

For each ego candidate $\tau^k_e$, let $\mathcal{I}^k$ be the set of critical agents selected from $\mathcal{G}_B$. For each $i\in\mathcal{I}^k$, let $\mathcal{A}^{\mathrm{nat}}_i$ denote the non-coerced natural alternative set of agent $i$, and let $\mathcal{R}^{\mathrm{safe}}_i(\tau^k_e)$ denote the set of ego-conditioned safe responses. The low-burden safe response set is
\begin{equation}
...
\end{equation}
where the small mass floor $\varepsilon_p$ prevents a learned predictor from hiding a safety-relevant root by assigning it zero probability. A candidate is non-coercively feasible only if every critical agent has at least one same-root low-burden safe response and $O_i(\tau^k_e)\ge\alpha$. This finite, root-indexed definition replaces an ill-defined set intersection between continuous trajectory samples and matches the implemented transport labels.
...
Each source is represented by stable typed roots and a causal dynamics residual. For root $m$, the decoder predicts bounded local longitudinal/lateral acceleration, jerk, and yaw-rate corrections $\delta u_{im}(t)$ around an analytic source prototype, and obtains position and velocity by integration. To prevent a bounded instantaneous control from accumulating into an unbounded eight-second displacement, the integrated residual obeys a source-conditioned trust region
\begin{equation}
    \left\|\delta p_{im}(T)\right\|_2 \le R_{s(m)},
    \qquad R_{\mathrm{OBS}}>R_{\mathrm{NEU}}>R_{\mathrm{PRIO}},
    \label{eq:natural_trust_region}
\end{equation}
where observational roots receive more fitting capacity while neutral and priority-preserving roots remain closer to their interpretable priors. The projection scales the complete control sequence before re-integration, so position and velocity remain mutually consistent.
...
\begin{aligned}
    ...
    +\lambda_{\mathrm{eff}}\mathcal{L}_{\mathrm{OBS\mbox{-}gain}}
    +\lambda_{\mathrm{pres}}\mathcal{L}_{\mathrm{NEU/PRIO\mbox{-}pres}}\\
    &+\lambda_{\mathrm{kin}}\mathcal{L}_{\mathrm{kin}}
    +\lambda_{\mathrm{mode}}\mathcal{L}_{\mathrm{usage}}
    +\lambda_{\mathrm{tr}}\mathcal{L}_{\mathrm{trust}} .
\end{aligned}
\end{equation}
Here $\mathcal{L}_{\mathrm{set}}$ contains source-restricted weighted set prediction and short-horizon terms; $\mathcal{L}_{\mathrm{OBS\mbox{-}gain}}$ requires the learned residual to improve difficult observational roots relative to the exact analytic basis; $\mathcal{L}_{\mathrm{NEU/PRIO\mbox{-}pres}}$ prevents the gain from erasing neutral or priority-preserving priors; $\mathcal{L}_{\mathrm{kin}}$ enforces finite-difference velocity, velocity--heading consistency, and smooth controls; $\mathcal{L}_{\mathrm{usage}}$ discourages root collapse; and $\mathcal{L}_{\mathrm{trust}}$ penalizes the pre-projection integrated endpoint when it approaches or exceeds the boundary in Eq.~\eqref{eq:natural_trust_region}. Using the pre-projection quantity is essential because a penalty on a radially hard-projected endpoint has zero radial gradient outside the feasible ball. Component claims are made only after same-seed one-factor ablations. Detailed construction is provided in Appendix~\ref{app:natural_alternatives}.
...
An engineering smoke run validates only data flow and numerical integrity. Decoder, loss, and residual-capacity claims require fresh controlled ablations with identical seeds, batches, initialization, and evaluator. Planner and selector claims require at least three seeds, paired scenario-level confidence intervals, no bypassed quality gate, and a natural-basis checkpoint that has already passed its gate. Downstream results are treated as mechanism probes rather than evidence when the natural basis is unverified.
...
\paragraph{False-Safe Rate (FSR).}
The fraction of collision-free selected ego plans that are certified as false-safe under counterfactual rollout:
\begin{equation}
    \mathrm{FSR}
    =
    \frac{
...
\end{equation}
A lower FSR means that the planner is less likely to hide risk by relying on another road user's costly ceding response.

\paragraph{Ceding Burden Score (CBS).}
The maximum burden imposed on any critical non-ego agent:
\begin{equation}
    \mathrm{CBS}
    =
    \frac{1}{N}
    \sum_{n=1}^{N}
    \max_{i\in\mathcal{I}^n}
    B_i^{\mathrm{prim}}(\tau_i^{\mathrm{cede}};\mathcal{S}^n,\tau^n_e).
\end{equation}
CBS measures how much conflict-resolution burden is transferred away from the ego vehicle.

\paragraph{Option Preservation Ratio (OPR).}
OPR is the average retained probability mass of same-root low-burden responses:
\begin{equation}
    \mathrm{OPR}
    =
    \frac{1}{N}
    \sum_{n=1}^{N}
    \frac{1}{|\mathcal{I}^n|}
    \sum_{i\in\mathcal{I}^n}
    \sum_m \widetilde p^n_{im}s^n_{im}.
\end{equation}
This finite root-indexed quantity is directly estimated by the transport model and avoids an undefined geometric intersection between independently sampled trajectory sets.

\paragraph{High-Burden Ceding Rate (HBCR).}
The fraction of episodes in which at least one non-ego agent must exceed the burden threshold to avoid conflict with the ego:
\begin{equation}
    \mathrm{HBCR}
    =
    \frac{1}{N}
    ...
    \mathbf{1}
    [
    \exists i\in\mathcal{I}^n:
    B_i^{\mathrm{prim}}(\tau_i^{\mathrm{cede}})>\beta_i(\mathcal{S}^n)
    ].
\end{equation}
...
\begin{table}[t]
\centering
\caption{Closed-loop planning performance on interaction-heavy scenarios. Lower is better for CR, FSR, CBS, and HBCR. Higher is better for EP and OPR.}
\label{tab:main_results}
\resizebox{\linewidth}{!}{
\begin{tabular}{lcccccc}
\toprule
Method & CR $\downarrow$ & EP $\uparrow$ & FSR $\downarrow$ & CBS $\downarrow$ & OPR $\uparrow$ & HBCR $\downarrow$ \\
\midrule
IDM/Lattice & -- & -- & -- & -- & -- & -- \\
...
We expect conventional planners to perform competitively on collision rate and ego progress, but to exhibit higher false-safe rate and ceding burden in dense interactions. COWP is designed to reduce FSR, CBS, and HBCR while maintaining comparable CR and EP.
...
\begin{tabular}{lcccc}
\toprule
Variant & FSR $\downarrow$ & CBS $\downarrow$ & OPR $\uparrow$ & EP $\uparrow$ \\
\midrule
Full COWP & -- & -- & -- & -- \\
...
The key expected trend is that using burden as a soft cost reduces burden only partially, while witness-level rejection more reliably suppresses false-safe candidates.
...
Qualitative results visualize the accepted ego trajectory, rejected false-safe candidates, the burdened agent, the natural alternative, the required ceding response, and the mechanism token. These visualizations demonstrate that COWP rejects plans not because they are slow or conservative, but because they transfer conflict-resolution responsibility to another road user.
...
\subsection{Macro Intent Set and Ego Candidate Generation}
\label{app:ego_candidates}

Each ego candidate is represented as a macro intent and a continuous trajectory:
\begin{equation}
    (\tau^k_e,a^k_e), \qquad
    \tau^k_e=\{x^k_e(t)\}_{t=1}^{T}.
\end{equation}
The macro intent set is
\begin{equation}
\begin{aligned}
    \mathcal{A}_{\mathrm{macro}}
    =
    \{
    &\mathrm{KeepLane},
    \mathrm{Yield},
...
\end{aligned}
\end{equation}
The set covers longitudinal negotiation, merge ordering, lane-change, and crossing behaviors.

For each macro intent, a nominal reference path $p^a(s)$ is extracted from the route and map topology. We sample terminal longitudinal offsets and speeds:
\begin{equation}
    (s_T,v_T)\sim \mathcal{Q}_a,
\end{equation}
where $\mathcal{Q}_a$ is intent-specific. A continuous candidate is generated by solving
\begin{equation}
\begin{aligned}
    \min_{\tau_e}\quad
    &
    w_r J_{\mathrm{route}}(\tau_e)
    +
    w_c J_{\mathrm{comfort}}(\tau_e)
    +
    w_p J_{\mathrm{progress}}(\tau_e)
    +
    w_l J_{\mathrm{lane}}(\tau_e) \\
    \mathrm{s.t.}\quad
    &
    x_e(0)=x_e, \\
    &
    |\dot{v}(t)|\le a^{\max}_e,\quad
    |\ddot{v}(t)|\le j^{\max}_e,\\
    &
    |\kappa(t)|\le \kappa^{\max}_e,\quad
    x_e(t)\in\mathcal{M}_{\mathrm{drivable}},\\
    &
    \tau_e \in \mathcal{C}_a .
\end{aligned}
\end{equation}
Here $\mathcal{C}_a$ contains intent-specific constraints, e.g., stopping before the conflict region, merging before or behind a target gap, or remaining in the current lane. We retain the top $K$ candidates according to the proposal cost and diversity in terminal state and macro intent.
...
\begin{equation}
    ...
    \sum_m w_i^m=1 .
\end{equation}
If calibrated coverage is required, we enlarge the sampled alternatives using adaptive conformal radii:
\begin{equation}
    ...
    \right\},
\end{equation}
where $q_{1-\delta}$ is estimated on a calibration stream.

\subsection{Natural Alternative Loss}
\label{app:natural_loss}

The natural alternative training objective follows the source-restricted set, effectiveness, preservation, physical-consistency, mode-usage, and trust-region decomposition in the main text. Let $\widetilde{d}_{im}=\|\widetilde{\delta p}_{im}(T)\|_2$ denote the integrated endpoint before radial projection and let $R_{s(m)}$ be its source budget. The dimensionless interior regularizer is
\begin{equation}
    \mathcal{L}_{\mathrm{trust}}
    =
    \frac{1}{|\mathcal{V}|}
    \sum_{(i,m)\in\mathcal{V}}
    \left[
      \frac{\widetilde{d}_{im}/R_{s(m)}-\eta}{1-\eta}
    \right]_+^2,
    \qquad 0<\eta<1,
\end{equation}
while the hard projection in Eq.~\eqref{eq:natural_trust_region} guarantees bounded integrated displacement. The loss is evaluated before projection so that over-budget controls retain a radial gradient toward the feasible interior. This normalized term avoids mixing metres, radians, and velocity units in a single unscaled state-space norm.
...
The burden threshold is context dependent but is not learned jointly with the certificate. We calibrate and freeze it on a held-out split:
\begin{equation}
    \beta_i(\mathcal{S})
    =Q_{1-\delta_\beta}
    \left(B_i^{\mathrm{nat}}\mid \mathrm{context}(\mathcal{S},i)\right),
\end{equation}
with a finite-sample conformal correction. Freezing $\beta_i$ prevents the model from explaining away coercive examples by increasing its own acceptance threshold.
...
\end{equation}
The optimized responses are generated by minimizing burden subject to safety:
\begin{equation}
\begin{aligned}
    \tau_i^{\mathrm{opt}}
    =
    \arg\min_{\tau_i}\quad
    &
    B_i^{\mathrm{prim}}(\tau_i;\mathcal{S},\tau^k_e) \\
    \mathrm{s.t.}\quad
    &
    \mathrm{Collide}(\tau^k_e,\tau_i)=0,\\
    &
    \mathrm{DynOK}(\tau_i)=1,\quad
    \mathrm{MapOK}(\tau_i)=1 .
\end{aligned}
\end{equation}
Emergency responses include hard braking or evasive stop primitives and are used only to identify whether collision freedom requires high burden.
...
Reject candidates with a high-confidence witness:
\begin{equation}
    \max_i
    P(W^k_{ei}=1)
    >
    ...
    \mathrm{Reject}(\tau^k_e).
\end{equation}
For uncertain witnesses, we retain the candidate but add a penalty:
\begin{equation}
...
\end{equation}
Candidates without high-confidence coercion are selected by minimum score.

\subsection{Conservative Fallback}
\label{app:fallback}

If no candidate survives the non-coercive feasibility filter, we execute a conservative fallback. The fallback is selected in the following order:
\begin{enumerate}
    \item \textbf{Controlled deceleration:} reduce speed while maintaining lane and comfort bounds.
    \item \textbf{Yield-before-conflict:} stop before the nearest conflict region if stopping distance is feasible.
    \item \textbf{Gap enlargement:} increase time headway to the critical agent.
    \item \textbf{Creep:} advance slowly only when the ego must improve visibility and does not enter the conflict region.
    \item \textbf{Emergency stop:} execute hard braking only if all comfort-bounded options are infeasible.
\end{enumerate}
The fallback is not optimized for progress; it is designed to remove ego-imposed conflict pressure until a non-coercive candidate becomes available.
