...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v16_3_pipeline_v9labels_seed2026}"
...
CONFIG_CANDIDATE_DIR="$(mktemp -d "$OUT_ROOT/configs/.candidate_v16_3.XXXXXX")"
...
"$PYTHON_BIN" -u -m cowp.scripts.42_write_run_provenance \
  ...
  --file "run_cowp_v16_3_dual_gpu.sh=$0" | tee "$OUT_ROOT/logs/run_provenance.log"
...
if [[ "$DETACH" == "1" && "${COWP_V16_3_DETACHED:-0}" != "1" ]]; then
  export COWP_V16_3_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v16.3] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "diagnose" ]]; then
  echo "[cowp_v16.3] stopped after diagnostics: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "natural" ]]; then
  echo "[cowp_v16.3] stopped after natural gate: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "transport" ]]; then
  echo "[cowp_v16.3] stopped after transport: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "planner" ]]; then
  echo "[cowp_v16.3] stopped after planner: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "offline" ]]; then
  echo "[cowp_v16.3] stopped after learned-offline verification: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "probe" ]]; then
  echo "[cowp_v16.3] stopped after probe: $OUT_ROOT"
  exit 0
fi
...
echo "[cowp_v16.3] complete: $OUT_ROOT"
