...
The observational branch $\mathcal{A}^{\mathrm{obs}}_i$ uses a multimodal trajectory decoder conditioned on the full scene to keep the alternatives close to the empirical driving distribution. The ego-neutral branch $\mathcal{A}^{\mathrm{neu}}_i$ predicts the agent's response under an intervention in which the ego does not impose the candidate conflict:
\begin{equation}
    \mathcal{A}^{\mathrm{neu}}_i
    \sim
    p_\theta(\tau_i \mid \mathcal{S}, do(\tau_e=\tau^{\mathrm{neu}}_e)).
\end{equation}
The priority-preserving branch $\mathcal{A}^{\mathrm{prio}}_i$ generates rule-constrained alternatives that preserve the agent's local priority and comfort-bounded motion.
...
\begin{equation}
    ...
    +
    \lambda_n \mathcal{L}_{\mathrm{neutral}}
    +
    \lambda_p \mathcal{L}_{\mathrm{prio}}
    +
    \lambda_d \mathcal{L}_{\mathrm{div}} ,
\end{equation}
where the four terms correspond to observed trajectory supervision, ego-neutral masked prediction, rule-constrained imitation, and diversity/coverage regularization. Detailed construction is provided in Appendix~\ref{app:natural_alternatives}.
...
The natural alternative training loss is
\begin{equation}
    \mathcal{L}_{\mathrm{nat}}
    =
    \lambda_o \mathcal{L}_{\mathrm{obs}}
