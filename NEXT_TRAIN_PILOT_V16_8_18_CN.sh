...
TRAIN_SCENARIO_INDEX="$INDEX_ROOT/scenario_training_locations.jsonl"
TRAIN_SCENARIO_INDEX_META="$INDEX_ROOT/scenario_training_locations.meta.json"

SCENARIO_TRAIN="$WOMD_ROOT/uncompressed/scenario/training/*.tfrecord*"
FRESH="$TRAIN_PILOT_ROOT/labels_train_v16_8_18"
...
VERDICT="$TRAIN_PILOT_ROOT/v16_8_18_train_pilot_verdict.json"
FP_FILE="$TRAIN_PILOT_ROOT/v16_8_18_code_fingerprint.sha256"
SPARSE_INTEGRITY="$TRAIN_PILOT_ROOT/sparse_label_build_integrity.json"
PIPELINE_STATUS="$TRAIN_PILOT_ROOT/train_pilot_pipeline_status.json"
...
Path(sys.argv[1]).write_text(json.dumps({
 'schema_version':'cowp_v16_8_18_train_pilot_pipeline_status_v1','pipeline_complete':False,
 'failure_stage':sys.argv[2],'return_code':int(sys.argv[3]),'composite_verdict_written':False,
 'next_action':'Repair/resume this pipeline failure before interpreting train-pilot support.'
...
if [[ "$FORCE_REBUILD_TRAIN_PILOT" == "1" ]]; then
  ...
  rm -f "$PROFILE" "$SUMMARY" "$CEILING_OLD" "$HARD_IDS" "$RANDOM_IDS" "$UNION_IDS" \
        ...
        "$CEILING_FRESH" "$VERDICT" "$FP_FILE" "$SPARSE_INTEGRITY" "$PIPELINE_STATUS"
fi
mkdir -p "$FRESH"
printf '%s\n' "$CODE_FP" > "$FP_FILE"
...
run scenario_location_index "$PYTHON_BIN" -m cowp.scripts.72_build_scenario_location_index \
  --proto-glob "$SCENARIO_TRAIN" --output "$TRAIN_SCENARIO_INDEX" --meta-output "$TRAIN_SCENARIO_INDEX_META" --reuse-if-valid
...
run build "$PYTHON_BIN" -m cowp.scripts.01_build_labels_from_proto \
  ...
  --index-jsonl "$TRAIN_SCENARIO_INDEX" --require-all-allowed-resolved \
...
run sparse_integrity "$PYTHON_BIN" -m cowp.scripts.71_validate_sparse_label_build \
  --labels-dir "$FRESH" --scene-ids "$UNION_IDS" --profile-jsonl "$PROFILE" --output "$SPARSE_INTEGRITY"
...
run model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support \
  ...
  --max-auditability-stratum-gap 0.03 --max-certificate-stratum-gap 0.08 --strict \
...
"$PYTHON_BIN" - "$VERDICT" "$CODE_FP" "$SUPERVISION" "$MODEL_SUPPORT" "$CAUSAL_AUDIT" "$NATURAL_DIAG" "$CEILING_FRESH" "$SPARSE_INTEGRITY" "$SUP_RC" "$MS_RC" <<'PY'
import json,sys
from pathlib import Path
out,fp,sup_p,ms_p,ca_p,nat_p,ceil_p,sparse_p,sup_rc,ms_rc=sys.argv[1:]
...
sparse=json.load(open(sparse_p,encoding='utf-8'))
...
checks={
  ...
  'protected_prio_coverage': float(nat.get('protected_prio_root_coverage',0.0)) >= 0.98,
...
payload={
  'schema_version':'cowp_v16_8_18_train_pilot_verdict_v4',
  'code_fingerprint_sha256':fp,
  'pass':passed,
  'recommend_full_rebuild':passed,
  'checks':checks,
  'sparse_build_integrity':sparse,
  'semantic_returncodes':{'supervision':int(sup_rc),'model_support':int(ms_rc)},
  'model_support_coverage':{'statistics':ms.get('coverage_statistics',{}),'by_stratum':ms.get('coverage_by_stratum',{})},
  'scene_rates':rates,
  'natural_support':{
    ...
    'protected_auditable_critical_agents':nat.get('protected_auditable_critical_agents'),
    'protected_without_prio_root':nat.get('protected_without_prio_root'),
    'protected_prio_root_coverage':nat.get('protected_prio_root_coverage'),
...
Path(sys.argv[1]).write_text(json.dumps({
 'schema_version':'cowp_v16_8_18_train_pilot_pipeline_status_v1','pipeline_complete':True,
