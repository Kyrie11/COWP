...
\end{equation}
where $\tau^k_e$ is a continuous ego trajectory and $a^k_e$ is a proposal descriptor. In the evaluated implementation, the bank is a finite set of deterministic kinematic primitives: constant-acceleration, comfort-bounded stop/creep, conflict-timing, terminal-state, and delayed lateral-offset trajectories. Interaction timing is covered by a boundary-consistent smooth multi-region bidirectional conflict-time envelope that proposes ego passage before the earliest plausible or after the latest plausible arrival of a nearby agent at each nominally reachable conflict region. Both ego and agent timing are referenced to first vehicle-envelope entry into the inflated conflict boundary, and each ego timing primitive is re-measured against its advertised target time. Synthetic primitives are screened for acceleration, deceleration, a prefix-robust jerk statistic, timing consistency, duplicate endpoints, non-negative terminal speed, and sampled lane-corridor compliance. Each candidate stores its proposal source, conflict region, timing side, target agent, target time, gap, solved initial acceleration, conflict-entry distance, and realized target-time error. The macro label and provenance describe how a primitive was generated; neither is itself evidence of route feasibility, right-of-way, successful negotiation, or non-coercive feasibility.
...
\paragraph{Proposal-sufficiency floor.}
Certificate quality and proposal sufficiency are audited independently.  Let
$A_n=\ind[\mathcal K^n_{\mathrm{prop,conv}}\neq\emptyset]$ indicate that the
fixed candidate bank contains a conventionally safe proposal, and let
$N_n=\ind[\mathcal K^n_{\mathrm{prop,ncf}}\neq\emptyset]$.  Because every NCF
candidate is conventionally safe, $N_n\le A_n$.
\begin{proposition}[Fixed-bank false-safe lower bound]
\label{prop:proposal_floor}
For any selector that returns a conventionally safe candidate whenever
$A_n=1$, its scene-level selected false-safe indicator $F_n$ satisfies
\begin{equation}
    F_n \ge A_n(1-N_n).
\end{equation}
Consequently, no certificate threshold, ranking loss, or fallback rule operating
only on the fixed bank can reduce the selected false-safe scene rate below
$\frac{1}{N}\sum_n A_n(1-N_n)$.  The analogous protected-priority lower bound is
computed by replacing $A_n,N_n$ with existence of a protected-eligible proposal
and a protected NCF proposal, respectively.
\end{proposition}
The result is bank-dependent rather than a universal impossibility statement:
it identifies when further threshold calibration is mathematically incapable of
meeting a gate and proposal refinement is required.  We therefore report
\texttt{AnyNCFSceneRate}, the conventional-without-NCF floor, protected
proposal coverage, NCF-Ret, and the selector's excess above the corresponding
proposal floor.  Full data reconstruction is authorized only after a paired,
scenario-matched proposal probe improves these quantities on an unbiased subset
without losing existing NCF scenes.
...
Our formulation reframes interactive safety from ``Can the ego avoid collision?'' to ``Can the ego remain safe without forcing others to absorb the conflict?'' This perspective exposes a class of planning failures that conventional benchmarks can overlook. The proposal-sufficiency bound further separates a weak certificate from a candidate bank that contains no admissible solution. Future work will extend non-coercive feasibility to pedestrian and cyclist interactions, distributional root-conditioned burden transport, certificate-guided refinement on top of stronger proposal planners, richer social conventions, and uncertainty-calibrated certificates under ego-induced interaction shift.
...
The current proposal bank contains: logged ego replay; constant-acceleration longitudinal primitives; smooth stop and creep primitives; conflict-region stopping margins; legacy ego-only conflict-arrival offsets; delayed lateral-offset primitives; and terminal speed/position variants.  We further use a \emph{Boundary-Consistent Smooth Robust Multi-Region Bidirectional Conflict-Time Envelope} (BCS-RMR-BCTE) proposal family.  The generator ranks only conflict regions that the nominal ego primitive actually reaches and measures ego arrival at the first entry of the vehicle envelope into the inflated conflict-region boundary, rather than at the region center.  For agent $j$ approaching region $r$, the current velocity ray is intersected with the same inflated boundary; bounded longitudinal acceleration around that entry distance gives a causal arrival envelope
\begin{equation}
    [t^-_{jr},t^+_{jr}],
\end{equation}
which uses only the observed current state and is clipped to a proposal-level uncertainty spread rather than interpreted as a formal reachable-set guarantee.  BCS-RMR-BCTE proposes the ego target times
\begin{equation}
    t^{\mathrm{before}}_{ejr}=t^-_{jr}-\Delta,
    \qquad
    t^{\mathrm{after}}_{ejr}=t^+_{jr}+\Delta.
\end{equation}
Let $d^{\mathrm{entry}}_r$ be the ego arc length from the observed current state to first entry of the inflated region boundary.  Instead of a constant-deceleration profile that can stop before a delayed requested arrival, the timing primitive uses
\begin{equation}
    s(t)=v_{e,0}t+c_2t^2+c_3t^3,
    \qquad
    s(T)=d^{\mathrm{entry}}_r,
    \qquad
    \ddot s(T)=0,
\end{equation}
with
\begin{equation}
    c_2=\frac{3(d^{\mathrm{entry}}_r-v_{e,0}T)}{2T^2},
    \qquad
    c_3=\frac{v_{e,0}T-d^{\mathrm{entry}}_r}{2T^3}.
\end{equation}
We reject profiles whose initial acceleration, constant jerk, or terminal speed violates the configured physical bounds.  The realized trajectory is then re-measured against the conflict boundary and retained only when
\begin{equation}
    |T^{\mathrm{entry}}_{e,r}-T|\le \epsilon_T,
\end{equation}
so a candidate cannot be labeled pass-before/pass-after while geometrically entering the conflict region at a materially different time.  Candidate quotas and deduplication over $(r,\mathrm{side},\mathrm{acceleration\ bin})$ preserve distinct interaction hypotheses as well as room for lane-change, stop, creep, and terminal alternatives.  The offline and online generators use the same smooth-arrival timing family, while candidate provenance records the entry distance and realized target-time error for source-specific auditing.  BCS-RMR-BCTE changes only proposal coverage: every generated trajectory remains subject to conventional safety and the root-conditioned non-coercive certificate, so adding proposals cannot turn an uncertified maneuver into a certified one.
