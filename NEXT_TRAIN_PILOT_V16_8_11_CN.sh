...
FRESH="$TRAIN_PILOT_ROOT/labels_train_v16_8_11"
...
VERDICT="$TRAIN_PILOT_ROOT/v16_8_11_train_pilot_verdict.json"
FP_FILE="$TRAIN_PILOT_ROOT/v16_8_11_code_fingerprint.sha256"
...
run choose_train_pilot "$PYTHON_BIN" -m cowp.scripts.45_diagnose_proposal_ceiling \
  ...
  --random-scene-ids "$RANDOM_IDS" --random-count "$RANDOM_COUNT" --random-exclude-hard-probe \
...
run validate_manifest "$PYTHON_BIN" -m cowp.scripts.63_validate_probe_manifest \
  ...
  --union-ids "$UNION_IDS" --expected-hard "$HARD_COUNT" --expected-random "$RANDOM_COUNT" \
  --output "$MANIFEST"
...
run model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support \
  --cache-dir "$FRESH" --sample-scenes 0 --min-class-examples 32 --min-source-examples 32 --strict \
...
"$PYTHON_BIN" - "$VERDICT" "$CODE_FP" "$SUPERVISION" "$MODEL_SUPPORT" "$CAUSAL_AUDIT" "$NATURAL_DIAG" "$CEILING_FRESH" <<'PY'
import json,sys
from pathlib import Path
out,fp,sup_p,ms_p,ca_p,nat_p,ceil_p=sys.argv[1:]
...
checks={
  ...
  # This is a support gate, not a publication metric. A 400-hard+800-random train
  # pilot should retain substantial NCF coverage before 22k-scene reconstruction.
  'candidate_any_ncf_support': float(rates.get('any_ncf',0.0)) >= 0.30,
}
passed=all(checks.values())
payload={
  'schema_version':'cowp_v16_8_11_train_pilot_verdict_v1',
  ...
  'scene_rates':rates,
  'natural_support':{
    'critical_agents':nat.get('critical_agents'),
