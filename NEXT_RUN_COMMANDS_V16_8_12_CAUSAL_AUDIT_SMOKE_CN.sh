...
export SMOKE_ROOT="${SMOKE_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_12_support_smoke}"
export HARD_COUNT="${HARD_COUNT:-48}" RANDOM_COUNT="${RANDOM_COUNT:-48}" LABEL_WORKERS="${LABEL_WORKERS:-24}" FORCE_REBUILD_SMOKE="${FORCE_REBUILD_SMOKE:-1}"
...
FRESH_LABELS="$SMOKE_ROOT/labels_val_v16_8_12"
...
BASE_SCREEN="$SMOKE_ROOT/base_screen_verdict.json"; VERDICT="$SMOKE_ROOT/v16_8_12_smoke_verdict.json"; FP_FILE="$SMOKE_ROOT/v16_8_12_code_fingerprint.sha256"
mkdir -p "$SMOKE_ROOT/logs"

SOURCE_HARD="$SOURCE_PROBE_ROOT/hard_scene_ids.txt"
SOURCE_RANDOM="$SOURCE_PROBE_ROOT/random_scene_ids.txt"; [[ -s "$SOURCE_RANDOM" ]] || SOURCE_RANDOM="$SOURCE_PROBE_ROOT/representative_random_scene_ids.txt"
...
[[ "$(wc -l < "$HARD_IDS")" -eq "$HARD_COUNT" ]] || { echo "smoke hard manifest shorter than HARD_COUNT" >&2; exit 3; }
[[ "$(wc -l < "$RANDOM_IDS")" -eq "$RANDOM_COUNT" ]] || { echo "smoke random manifest shorter than RANDOM_COUNT" >&2; exit 3; }
[[ "$(sort -u "$UNION_IDS" | wc -l)" -eq $((HARD_COUNT + RANDOM_COUNT)) ]] || { echo "smoke manifest is not disjoint/unique" >&2; exit 3; }
...
run_semantic model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support --cache-dir "$FRESH_LABELS" --sample-scenes 0 --min-class-examples 8 --min-source-examples 8 --output "$MODEL_SUPPORT" --strict; MS_RC=$?
...
checks={
  ...
  'natural_rootless_zero': int(nat.get('rootless_critical_agents',-1))==0,
  'natural_lt2_low_burden_zero': int(nat.get('critical_agents_with_lt2_low_burden_roots',-1))==0,
}
passed=all(checks.values())
payload={
 'schema_version':'cowp_v16_8_12_smoke_composite_verdict_v1', 'code_fingerprint_sha256':fp,
 ...
 'base_screen':base,
 'natural_support':{k:nat.get(k) for k in ('critical_agents','rootless_critical_agents','critical_agents_with_lt2_low_burden_roots','rootless_dominant_rejection','lt2_low_burden_dominant_rejection','priority_rejection_reason_counts','map_rejected_min_distance_summary_m','map_rejected_max_distance_summary_m')},
