...
# v16.8.37: Recourse Returnability Bridge (RRB)
# Preregistered branch after v16.8.36 selector/frontier failure: can a recovery
# intervention be certified by a causal one-replan witness that returns to the
# unchanged full-conventional feasible set?
#
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_37_recourse_returnability_bridge}"
...
for method in [x.strip() for x in methods.split(',') if x.strip()]:
    if method!='cowp_recourse_returnability_bridge':
        raise SystemExit(f'ERROR: {method} is not a v16.8.37 promotion candidate')
    g=gates.get('recourse_returnability_bridge')
...
analyze_set() {
  ...
  local args=(--cowp "$cowp" --rvr "$rvr" --stage "$stage" --development-selected \
    --recourse-returnability-bridge "$OUT_ROOT/${tag}_cowp_recourse_returnability_bridge_merged.json" \
    --output "$OUT_ROOT/${tag}_recourse_returnability_bridge_analysis.json")
  if [[ "$stage" == "counterfactual48" ]]; then
    args+=(--v33-rosh "$REF_CF48_V33_ROSH" --v35-control-projected-spectrum "$REF_CF48_V35_CPOSH" --v36-recovery-frontier "$REF_CF48_V36_FRONTIER")
  fi
  python -m cowp.scripts.90_analyze_recourse_returnability_bridge "${args[@]}"
}
...
case "$MODE" in
  sanity)
    ...
    ensure_ids
    echo "v16.8.37 focused semantic/integrity sanity passed"
    ;;
  make_ids) ensure_ids ;;
  build_tfindex)
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v37" "$CF48_IDS" "${METHODS:-cowp_recourse_returnability_bridge}" ;;
  analyze_counterfactual48)
    analyze_set "counterfactual48" "counterfactual48_v37" "$REF_CF48_COWP" "$REF_CF48_RVR" ;;
  fresh37_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_recourse_returnability_bridge}"
    require_gate_pass "$OUT_ROOT/counterfactual48_v37_recourse_returnability_bridge_analysis.json" "$promoted" "counterfactual48"
    run_parallel_methods "fresh37_v37" "$FRESH37_IDS" "$promoted" ;;
  analyze_fresh37)
    analyze_set "fresh37" "fresh37_v37" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR" ;;
  confirm200_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_recourse_returnability_bridge}"
    require_gate_pass "$OUT_ROOT/fresh37_v37_recourse_returnability_bridge_analysis.json" "$promoted" "fresh37"
...
STOP here unless preregistered_gate.recourse_returnability_bridge.pass == true.
The six Stage-1 GO conditions are unchanged from v16.8.33--36.

Only after Stage-1 pass:
  PROMOTED_METHODS=cowp_recourse_returnability_bridge $0 fresh37_parallel2
  $0 analyze_fresh37

Only after fresh37 pass:
  PROMOTED_METHODS=cowp_recourse_returnability_bridge $0 confirm200_parallel2
  $0 analyze_confirm200

Do NOT tune returnability depth, profile thresholds, weights, or dwell time after a failed gate.
V37 does not add proposal primitives; if this branch fails, move to genuine reachable-support construction.
EOF
    ;;
esac
