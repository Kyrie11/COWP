...
# COWP v16.8.10 promotion driver.
...
export SMOKE_ROOT="${SMOKE_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_10_support_smoke}"
export PROBE_ROOT="${PROBE_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_10_support_strict_probe}"
export FULL_ROOT="${FULL_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_10_full}"
...
Modes:
  ...
  full-core     Build full fresh labels + tensor caches only after strict verdict authorizes it; Waymax replay deferred.
...
Recommended order:
  preflight -> smoke -> fastpath-ab -> strict -> full-core -> outcomes
USAGE
}
...
strict(){
  ...
  export LABEL_WORKERS="${LABEL_WORKERS:-24}"
  # A changed code fingerprint must never reuse old labels.
