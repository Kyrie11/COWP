...
"$PYTHON_BIN" -u -m cowp.scripts.48_gate_waymax_probe_v16_8_4 \
  --reference "$PROBE_REF" --candidate "$PROBE_COWP" --min-rollouts "$MIN_PROBE_ROLLOUTS" \
  --output "$OUT_ROOT/eval/probe/promotion_gate_v16_8_9.json" \
  | tee "$OUT_ROOT/logs/promotion_gate_v16_8_9.log"
