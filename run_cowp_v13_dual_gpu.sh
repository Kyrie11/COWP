...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v13_temporal_riot_probe100_seed2026}"
...
NATURAL_EPOCHS="${NATURAL_EPOCHS:-30}"
...
cp configs/label_cowp_v13.yaml "$OUT_ROOT/configs/label_cowp_v13.yaml"
cp configs/label_cowp_v12_pareto_ablation.yaml "$OUT_ROOT/configs/label_cowp_v12_pareto_ablation.yaml"
cp configs/label_cowp_v12_pairmax_ablation.yaml "$OUT_ROOT/configs/label_cowp_v12_pairmax_ablation.yaml"
cp configs/train_cowp_v13.yaml "$OUT_ROOT/configs/train_cowp_v13.yaml"
cp configs/eval_cowp_v13.yaml "$OUT_ROOT/configs/eval_cowp_v13.yaml"
sed -i -E "0,/^  seed:/{s/^  seed:.*/  seed: ${TRAIN_SEED}/}" "$OUT_ROOT/configs/train_cowp_v13.yaml"
LABEL_CFG="$OUT_ROOT/configs/label_cowp_v13.yaml"
PARETO_CFG="$OUT_ROOT/configs/label_cowp_v12_pareto_ablation.yaml"
PAIRMAX_CFG="$OUT_ROOT/configs/label_cowp_v12_pairmax_ablation.yaml"
TRAIN_CFG="$OUT_ROOT/configs/train_cowp_v13.yaml"
EVAL_CFG="$OUT_ROOT/configs/eval_cowp_v13.yaml"

if [[ "$DETACH" == "1" && "${COWP_V13_DETACHED:-0}" != "1" ]]; then
  export COWP_V13_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v13] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
if [[ "$RUN_DIAGNOSE" == "1" ]]; then
  ...
  logrun diagnose_alignment_train "$PYTHON_BIN" -u -m cowp.scripts.33_diagnose_cache_alignment \
    ...
    --max-scenes "${ALIGNMENT_DIAG_SCENES:-2000}" --output "$OUT_ROOT/eval/cache_alignment_train.json" & d2=$!
  logrun diagnose_alignment_val "$PYTHON_BIN" -u -m cowp.scripts.33_diagnose_cache_alignment \
    ...
    --max-scenes "${ALIGNMENT_DIAG_SCENES:-2000}" --output "$OUT_ROOT/eval/cache_alignment_val.json" & d3=$!
  logrun diagnose_natural_oracle_val "$PYTHON_BIN" -u -m cowp.scripts.34_diagnose_natural_oracles \
    --cache-dir "$RAW_VAL_CACHE" --max-scenes "${ORACLE_DIAG_SCENES:-2000}" \
...
if [[ "$RUN_NATURAL" == "1" ]]; then
  if best_natural >/dev/null && [[ "$FORCE_TRAIN" != "1" ]]; then
    echo "[natural] keep existing $(best_natural)"
  else
    init_args=()
    ...
    logrun train_natural_ddp env CUDA_VISIBLE_DEVICES="$TRAIN_VISIBLE_DEVICES" \
      ...
      --lr-scheduler plateau --min-lr 5e-6 --save-every 2 \
      --no-positive-oversampling --amp --fused-adamw "${init_args[@]}"
  fi
fi
...
if [[ ! -s "$ORACLE_REPORT" ]]; then
  logrun diagnose_natural_oracle_val "$PYTHON_BIN" -u -m cowp.scripts.34_diagnose_natural_oracles \
    --cache-dir "$RAW_VAL_CACHE" --max-scenes "${ORACLE_DIAG_SCENES:-2000}" --output "$ORACLE_REPORT"
fi
logrun gate_natural_basis "$PYTHON_BIN" -u -m cowp.scripts.32_gate_natural_basis \
  ...
  --max-set-minade-m 12.0 --max-branch-minade-m 15.0 \
  --max-neutral-minade-m 15.0 --max-priority-minade-m 15.0
if [[ "$STOP_AFTER_STAGE" == "natural" ]]; then
  echo "[cowp_v13] stopped after natural gate: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "transport" ]]; then
  echo "[cowp_v13] stopped after transport: $OUT_ROOT"
  exit 0
fi
...
[[ -s "$CKPT" ]] || { echo "No v12 planner checkpoint. Set CKPT or enable RUN_PLANNER." >&2; exit 2; }
echo "[checkpoint] $CKPT"
if [[ "$STOP_AFTER_STAGE" == "planner" ]]; then
  echo "[cowp_v13] stopped after planner: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "offline" ]]; then
  echo "[cowp_v13] stopped after learned-offline verification: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "probe" ]]; then
  echo "[cowp_v13] stopped after probe: $OUT_ROOT"
  exit 0
fi
...
echo "[cowp_v13] complete: $OUT_ROOT"
