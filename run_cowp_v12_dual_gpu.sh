...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v12_riot_probe100_seed2026}"
...
NATURAL_EPOCHS="${NATURAL_EPOCHS:-8}"
...
cp configs/label_cowp_v12.yaml "$OUT_ROOT/configs/label_cowp_v12.yaml"
...
cp configs/train_cowp_v12.yaml "$OUT_ROOT/configs/train_cowp_v12.yaml"
cp configs/eval_cowp_v12.yaml "$OUT_ROOT/configs/eval_cowp_v12.yaml"
sed -i -E "0,/^  seed:/{s/^  seed:.*/  seed: ${TRAIN_SEED}/}" "$OUT_ROOT/configs/train_cowp_v12.yaml"
LABEL_CFG="$OUT_ROOT/configs/label_cowp_v12.yaml"
...
TRAIN_CFG="$OUT_ROOT/configs/train_cowp_v12.yaml"
EVAL_CFG="$OUT_ROOT/configs/eval_cowp_v12.yaml"

if [[ "$DETACH" == "1" && "${COWP_V12_DETACHED:-0}" != "1" ]]; then
  export COWP_V12_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v12] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
echo "[cowp_v12] complete: $OUT_ROOT"
