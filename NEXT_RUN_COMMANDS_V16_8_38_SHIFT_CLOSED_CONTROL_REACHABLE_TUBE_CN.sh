...
# v16.8.38: Shift-Closed Control-Reachable Recovery Tube (SCRT)
# Preregistered branch after v16.8.37's high-precision/low-recall returnability
# failure: construct explicit support inside the existing controller-reachable
# set, then require a full physical tube and one-step shift closure.
#
...
# candidate families, common controller, outcome settings, and the 8 s
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_38_shift_closed_control_reachable_tube}"
...
for method in [x.strip() for x in methods.split(',') if x.strip()]:
    if method!='cowp_shift_closed_control_reachable_tube':
        raise SystemExit(f'ERROR: {method} is not a v16.8.38 promotion candidate')
    g=gates.get('shift_closed_control_reachable_tube')
...
analyze_set() {
  ...
  local args=(--cowp "$cowp" --rvr "$rvr" --stage "$stage" --development-selected \
    --shift-closed-control-reachable-tube "$OUT_ROOT/${tag}_cowp_shift_closed_control_reachable_tube_merged.json" \
    --output "$OUT_ROOT/${tag}_shift_closed_control_reachable_tube_analysis.json")
  ...
  python -m cowp.scripts.91_analyze_shift_closed_control_reachable_tube "${args[@]}"
}
...
case "$MODE" in
  sanity)
    python -m py_compile cowp/waymax_eval/policy_wrapper.py cowp/waymax_eval/rollout.py cowp/waymax_eval/metrics_cowp.py \
      ...
      cowp/scripts/91_analyze_shift_closed_control_reachable_tube.py
    python -m pytest -q \
    ...
    ensure_ids
    echo "v16.8.38 focused semantic/integrity sanity passed"
    ;;
  make_ids) ensure_ids ;;
  build_tfindex)
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v38" "$CF48_IDS" "${METHODS:-cowp_shift_closed_control_reachable_tube}" ;;
  analyze_counterfactual48)
    analyze_set "counterfactual48" "counterfactual48_v38" "$REF_CF48_COWP" "$REF_CF48_RVR" ;;
  fresh37_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_shift_closed_control_reachable_tube}"
    require_gate_pass "$OUT_ROOT/counterfactual48_v38_shift_closed_control_reachable_tube_analysis.json" "$promoted" "counterfactual48"
    run_parallel_methods "fresh37_v38" "$FRESH37_IDS" "$promoted" ;;
  analyze_fresh37)
    analyze_set "fresh37" "fresh37_v38" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR" ;;
  confirm200_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_shift_closed_control_reachable_tube}"
    require_gate_pass "$OUT_ROOT/fresh37_v38_shift_closed_control_reachable_tube_analysis.json" "$promoted" "fresh37"
...
STOP here unless preregistered_gate.shift_closed_control_reachable_tube.pass == true.
The six Stage-1 GO conditions are unchanged from v16.8.33--37.

Only after Stage-1 pass:
  PROMOTED_METHODS=cowp_shift_closed_control_reachable_tube $0 fresh37_parallel2
  $0 analyze_fresh37

Only after fresh37 pass:
  PROMOTED_METHODS=cowp_shift_closed_control_reachable_tube $0 confirm200_parallel2
  $0 analyze_confirm200

Do NOT relax the tube certificate, shorten the frozen 8 s collision contract,
add weighted profile/risk scores, stack lookahead depth, or introduce dwell time
after a failed gate. A nonzero lifted-support rate is a mechanism-attribution
diagnostic, not a post-hoc seventh promotion threshold.
EOF
    ;;
esac
