...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v11_bcot_probe100_seed2026}"
...
DEFAULT_WITNESS_THRESHOLD="${WITNESS_THRESHOLD:-0.18}"
WITNESS_SWEEP="${WITNESS_SWEEP:-0.05,0.08,0.10,0.12,0.15,0.18,0.20,0.25,0.30,0.35,0.40,0.50,0.60}"
...
cp configs/label_cowp_v11.yaml "$OUT_ROOT/configs/label_cowp_v11.yaml"
cp configs/label_cowp_v11_pareto_ablation.yaml "$OUT_ROOT/configs/label_cowp_v11_pareto_ablation.yaml"
cp configs/label_cowp_v11_pairmax_ablation.yaml "$OUT_ROOT/configs/label_cowp_v11_pairmax_ablation.yaml"
cp configs/train_cowp_v11.yaml "$OUT_ROOT/configs/train_cowp_v11.yaml"
cp configs/eval_cowp_v11.yaml "$OUT_ROOT/configs/eval_cowp_v11.yaml"
sed -i -E "0,/^  seed:/{s/^  seed:.*/  seed: ${TRAIN_SEED}/}" "$OUT_ROOT/configs/train_cowp_v11.yaml"
LABEL_CFG="$OUT_ROOT/configs/label_cowp_v11.yaml"
PARETO_CFG="$OUT_ROOT/configs/label_cowp_v11_pareto_ablation.yaml"
PAIRMAX_CFG="$OUT_ROOT/configs/label_cowp_v11_pairmax_ablation.yaml"
TRAIN_CFG="$OUT_ROOT/configs/train_cowp_v11.yaml"
EVAL_CFG="$OUT_ROOT/configs/eval_cowp_v11.yaml"

if [[ "$DETACH" == "1" && "${COWP_V11_DETACHED:-0}" != "1" ]]; then
  export COWP_V11_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v11] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
if [[ "$RUN_TRANSPORT" == "1" && "$REQUIRE_INIT_CKPT" == "1" && ! -s "$INIT_CKPT" ]]; then
  echo "Required v10 initialization checkpoint is missing: $INIT_CKPT" >&2
...
[[ -s "$CKPT" ]] || { echo "No v11 planner checkpoint. Set CKPT or enable RUN_PLANNER." >&2; exit 2; }
...
if [[ "$RUN_OFFLINE" == "1" ]]; then
  ...
  if [[ "$RUN_PAIRMAX_ABLATION" == "1" ]] && ! json_valid "$pairmax_offline"; then
    logrun learned_offline_pairmax env CUDA_VISIBLE_DEVICES="$GPU1" \
      "$PYTHON_BIN" -u -m cowp.scripts.04_eval_closed_loop \
      --mode learned_offline --methods cowp \
      --checkpoint "$CKPT" --cache-dir "$VAL_CACHE" \
  ...
  logrun calibrate_witness "$PYTHON_BIN" -u -m cowp.scripts.18_calibrate_witness_threshold \
    ...
    --min-ncf-recall 0.30 --max-fallback 0.25
  logrun diagnose_bcot_result "$PYTHON_BIN" -u -m cowp.scripts.30_diagnose_bcot_result \
    --input "$combined" --method cowp \
    --calibration-json "$OUT_ROOT/eval/learned_offline/witness_calibration.json" \
    --output "$OUT_ROOT/eval/learned_offline/bcot_readiness.json"
  # Stop before Waymax unless both pair-level transport and the candidate-level
  # BCOT budget are informative and the feasible set no longer collapses.
  logrun verify_mechanism "$PYTHON_BIN" -u -m cowp.scripts.25_verify_mechanism_effect \
    ...
    --min-witness-auprc 0.60 --min-bcot-auprc 0.65 --min-accepted-rate 0.10 --max-fallback 0.25 \
...
echo "[cowp_v11] complete: $OUT_ROOT"
