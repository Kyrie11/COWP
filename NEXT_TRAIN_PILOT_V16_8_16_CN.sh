...
FRESH="$TRAIN_PILOT_ROOT/labels_train_v16_8_16"
...
VERDICT="$TRAIN_PILOT_ROOT/v16_8_16_train_pilot_verdict.json"
FP_FILE="$TRAIN_PILOT_ROOT/v16_8_16_code_fingerprint.sha256"
...
run model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support \
  --cache-dir "$FRESH" --sample-scenes 0 --min-class-examples 32 --min-source-examples 32 --max-unauditable-critical-rate 0.01 --min-certificate-complete-scene-rate 0.98 --min-protected-prio-coverage 0.98 --strict \
...
payload={
  'schema_version':'cowp_v16_8_16_train_pilot_verdict_v3',
