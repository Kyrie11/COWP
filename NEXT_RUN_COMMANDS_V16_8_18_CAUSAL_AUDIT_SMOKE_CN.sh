...
FRESH_LABELS="$SMOKE_ROOT/labels_val_v16_8_18"
...
BASE_SCREEN="$SMOKE_ROOT/base_screen_verdict.json"; VERDICT="$SMOKE_ROOT/v16_8_18_smoke_verdict.json"; FP_FILE="$SMOKE_ROOT/v16_8_18_code_fingerprint.sha256"
SPARSE_INTEGRITY="$SMOKE_ROOT/sparse_label_build_integrity.json"
PIPELINE_STATUS="$SMOKE_ROOT/smoke_pipeline_status.json"
...
Path(sys.argv[1]).write_text(json.dumps({
  ...
  'next_action':'Repair/resume the pipeline failure before interpreting model-support or semantic-equivalence gates.',
...
# v16.8.18: derive the stress manifest from the CURRENT old validation cache.
...
CURRENT_STAGE="manifest_selection"
...
if [[ "$FORCE_REBUILD_SMOKE" == "1" ]]; then
  rm -rf "$FRESH_LABELS"
  rm -f "$PROFILE" "$PROFILE_SUMMARY" "$PAIRED" "$ABLATION" "$AUDIT" "$SUPERVISION" "$MODEL_SUPPORT" "$NATURAL_DIAG" "$BASE_SCREEN" "$VERDICT" "$FP_FILE" "$SPARSE_INTEGRITY" "$PIPELINE_STATUS"
fi
...
CURRENT_STAGE="build_fresh_labels"
run build_fresh "$PYTHON_BIN" -m cowp.scripts.01_build_labels_from_proto --data-config configs/data.yaml --label-config "$LABEL_CFG" --proto-glob "$SCENARIO_VAL" --output-dir "$FRESH_LABELS" --allow-scenario-ids "$UNION_IDS" --index-jsonl "$VAL_SCENARIO_INDEX" --require-all-allowed-resolved --num-workers "$LABEL_WORKERS" --start-method forkserver --max-pending-multiplier 2 --no-compress --skip-existing --profile-jsonl "$PROFILE" --skip-diagnostics --cpu-only
CURRENT_STAGE="validate_sparse_build"
run sparse_integrity "$PYTHON_BIN" -m cowp.scripts.71_validate_sparse_label_build --labels-dir "$FRESH_LABELS" --scene-ids "$UNION_IDS" --profile-jsonl "$PROFILE" --output "$SPARSE_INTEGRITY"
...
run_semantic model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support \
  ...
  --min-protected-prio-coverage 0.95 --coverage-gate-mode wilson_gross_failure \
  --hard-scene-ids "$HARD_IDS" --random-scene-ids "$RANDOM_IDS" \
...
"$PYTHON_BIN" - "$VERDICT" "$CODE_FP" "$BASE_SCREEN" "$SUPERVISION" "$MODEL_SUPPORT" "$NATURAL_DIAG" "$SPARSE_INTEGRITY" "$SUP_RC" "$MS_RC" "$SCREEN_RC" <<'PY'
import json,sys
from pathlib import Path
...
min_prio_coverage=0.95
msc=ms.get('checks',{}) or {}
checks={
  'sparse_label_build_complete': bool(sparse.get('pass',False)),
  ...
  'protected_prio_coverage': float(nat.get('protected_prio_root_coverage',0.0)) >= min_prio_coverage,
}
passed=all(checks.values())
payload={
 'schema_version':'cowp_v16_8_18_smoke_composite_verdict_v4', 'code_fingerprint_sha256':fp,
 'pass':passed, 'recommend_strict_probe':passed, 'recommend_full_rebuild':False,
 'checks':checks, 'sparse_build_integrity':sparse, 'semantic_returncodes':{'supervision':int(sup_rc),'model_support':int(ms_rc),'screen':int(screen_rc)},
 'base_screen':base,
 'model_support_coverage':{'thresholds':{k:ms.get(k) for k in ('max_unauditable_critical_rate','min_certificate_complete_scene_rate','min_protected_prio_coverage','coverage_gate_mode','max_auditability_stratum_gap','max_certificate_stratum_gap')},'statistics':ms.get('coverage_statistics',{}),'by_stratum':ms.get('coverage_by_stratum',{})},
...
Path(sys.argv[1]).write_text(json.dumps({
  ...
  'recommend_strict_probe':bool(verdict.get('recommend_strict_probe',False)),
