...
# Reliability-repair release only. The uploaded V40 changed all-horizon shift
# terminal semantics inside interval completion: LOWER_ALL/UPPER_ALL appended a
# nominal edge instead of the V39 endpoint edge. V41 centralizes and restores the
# V39 rule. No new proposal, threshold, controller limit, certificate, selector,
# score, dataset, or learned component is introduced.
#
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_41_shift_closure_semantic_fidelity_repair}"
...
for method in [x.strip() for x in methods.split(',') if x.strip()]:
    if method!='cowp_shift_closed_first_action_viability_interval':
        raise SystemExit(f'ERROR: {method} is not the repaired v16.8.41 promotion candidate')
    g=gates.get('shift_closure_semantic_fidelity_repair')
...
analyze_set() {
  ...
  local args=(--cowp "$cowp" --rvr "$rvr" --stage "$stage" --development-selected \
    --shift-closure-semantic-fidelity-repair "$OUT_ROOT/${tag}_cowp_shift_closed_first_action_viability_interval_merged.json" \
    --output "$OUT_ROOT/${tag}_shift_closure_semantic_fidelity_repair_analysis.json")
  ...
  python -m cowp.scripts.94_analyze_shift_closure_semantic_fidelity_repair "${args[@]}"
}
...
case "$MODE" in
  sanity)
    python -m py_compile cowp/waymax_eval/policy_wrapper.py cowp/waymax_eval/rollout.py cowp/waymax_eval/metrics_cowp.py \
      ...
      cowp/scripts/92_analyze_conflict_window_control_reachable_tube.py cowp/scripts/93_analyze_shift_closed_first_action_viability_interval.py cowp/scripts/94_analyze_shift_closure_semantic_fidelity_repair.py
    python -m pytest -q \
    ...
    ensure_ids
    echo "v16.8.41 reliability-repair semantic/integrity sanity passed"
    ;;
  make_ids) ensure_ids ;;
  build_tfindex)
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v41" "$CF48_IDS" "${METHODS:-cowp_shift_closed_first_action_viability_interval}" ;;
  analyze_counterfactual48)
    analyze_set "counterfactual48" "counterfactual48_v41" "$REF_CF48_COWP" "$REF_CF48_RVR" ;;
  fresh37_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_shift_closed_first_action_viability_interval}"
    require_gate_pass "$OUT_ROOT/counterfactual48_v41_shift_closure_semantic_fidelity_repair_analysis.json" "$promoted" "counterfactual48"
    run_parallel_methods "fresh37_v41" "$FRESH37_IDS" "$promoted" ;;
  analyze_fresh37)
    analyze_set "fresh37" "fresh37_v41" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR" ;;
  confirm200_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_shift_closed_first_action_viability_interval}"
    require_gate_pass "$OUT_ROOT/fresh37_v41_shift_closure_semantic_fidelity_repair_analysis.json" "$promoted" "fresh37"
  ...
  help|*)
    cat <<EOF
v16.8.41 reliability-repair rerun order
...
STOP here unless preregistered_gate.shift_closure_semantic_fidelity_repair.pass == true.
The six Stage-1 GO conditions remain unchanged. V41 is a corrected rerun of V40, not a new mechanism.
...
Do not use the uploaded V40 outcomes for algorithm attribution. Run this repaired
version through equivalence16 and counterfactual48 first. Only after the repaired
result passes reliability may its unchanged six-item gate be interpreted. Do not
add a new V42 mechanism in the same run or relax any gate/certificate.
EOF
    ;;
esac
