...
# COWP v16.8.12 dataset-support promotion driver.
...
export FULL_ROOT="${FULL_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_12_full}"
...
SMOKE_VERDICT="$SMOKE_ROOT/v16_8_12_smoke_verdict.json"
STRICT_VERDICT="$PROBE_ROOT/v16_8_12_strict_verdict.json"
TRAIN_PILOT_VERDICT="$TRAIN_PILOT_ROOT/v16_8_12_train_pilot_verdict.json"
...
smoke(){
  ...
  set +e
  bash NEXT_RUN_COMMANDS_V16_8_12_CAUSAL_AUDIT_SMOKE_CN.sh
  local rc=$?
  set -e
  [[ -s "$SMOKE_ROOT/v16_8_12_smoke_verdict.json" ]] && cat "$SMOKE_ROOT/v16_8_12_smoke_verdict.json" || true
  return "$rc"
}

fastpath_ab(){
  local labels="$SMOKE_ROOT/labels_val_v16_8_12"
...
PY
  ...
  set +e
  bash NEXT_RUN_COMMANDS_V16_8_12_STRICT_PROPOSAL_PROBE_CN.sh
  local rc=$?
  set -e
  if [[ "$rc" -ne 0 ]]; then
...
PY
  TRAIN_PILOT_ROOT="$TRAIN_PILOT_ROOT" SOURCE_DATA_ROOT="$SOURCE_DATA_ROOT" WOMD_ROOT="$WOMD_ROOT" \
    bash NEXT_TRAIN_PILOT_V16_8_12_CN.sh
}
