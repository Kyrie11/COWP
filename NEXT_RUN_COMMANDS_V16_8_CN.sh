...
# 本脚本复用 raw Waymax cache，但要求先由 PREPARE_COWP_V16_8_OVERLAY_CN.sh
# 生成 root-conditioned transport v16.8 sidecar；不会静默回退到旧 v9 transport 标签。
...
if [[ "$BACKGROUND" == "1" && "${COWP_V16_8_BACKGROUND_CHILD:-0}" != "1" ]]; then
  export COWP_V16_8_BACKGROUND_CHILD=1
  nohup env BACKGROUND=0 bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 < /dev/null &
  pid=$!
  printf '%s\n' "$pid" > "$OUT_ROOT/logs/driver.pid"
  echo "[cowp_v16.8] background pid=$pid"
  echo "[cowp_v16.8] log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
CACHE_REUSE_REPORT="$OUT_ROOT/eval/cache_reuse_gate_v16_8.json"
if [[ "${FORCE_CACHE_AUDIT:-0}" == "1" || ! -s "$CACHE_REUSE_REPORT" ]]; then
  python -u -m cowp.scripts.38_gate_cache_reuse \
    ...
    | tee "$OUT_ROOT/logs/cache_reuse_gate_v16_8.log"
fi
...
bash run_cowp_v16_8_dual_gpu.sh
