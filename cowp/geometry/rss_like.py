...
def same_heading_gap_violation_bool(
    rear_traj: np.ndarray,
    front_traj: np.ndarray,
    heading_tolerance: float = np.deg2rad(35),
    rho: float = 0.5,
    a_max_accel: float = 2.0,
    b_min: float = 3.0,
    b_max: float = 6.0,
    min_gap: float = 2.0,
    lateral_margin: float = 0.75,
) -> bool:
    """Boolean-only equivalent of :func:`same_heading_gap_violation`."""
    t = min(len(rear_traj), len(front_traj))
    if t == 0:
        return False
    rear = np.asarray(rear_traj[:t], dtype=np.float32)
    ...
    d_safe = np.maximum(d_safe, min_gap).astype(np.float32)
    active = valid_heading & valid_gap & valid_lateral
    return bool(np.any(active & (longitudinal_gap < d_safe)))
