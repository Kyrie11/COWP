...
def policy_diagnostic_scenario_rows(rollouts: list[dict]) -> list[dict]:
    ...
    for idx, item in enumerate(rollouts):
        ...
        rec: dict[str, object] = {
            ...
            "no_conventional_step_rate": float(sum(x in {"no_conventional_use_least_coercive_valid", "no_conventional_use_recursive_viability", "no_conventional_use_rvr_pareto_guard", "no_conventional_use_successor_option_viability"} for x in reasons) / max(n, 1)),
