...
def policy_diagnostic_scenario_rows(rollouts: list[dict]) -> list[dict]:
    ...
    for idx, item in enumerate(rollouts):
        ...
        rec: dict[str, object] = {
            ...
            "verified_root_conditioned_recourse_set_operator_step_rate": float(sum(x == "no_conventional_use_verified_root_conditioned_recourse_set_operator" for x in reasons) / max(n, 1)),
            ...
            "recovery_tube_probe_step_rate": _mean("recovery_tube_probe_used"),
            "recovery_tube_certificate_step_rate": _mean("recovery_tube_selected"),
            "recovery_tube_action_change_step_rate": _mean("recovery_tube_action_changed"),
            "recovery_tube_lifted_selection_rate_on_certified_steps": _mean(
            ...
            "recovery_tube_interaction_attempt_step_rate": _mean(
                "recovery_tube_interaction_response_attempted"
            ),
            "recovery_tube_interaction_selection_rate_on_certified_steps": _mean(
                "recovery_tube_selected_is_interaction_response",
                mask=np.asarray([bool(r.get("recovery_tube_selected", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_support_agents_total_on_attempts": _mean(
                "recovery_tube_interaction_support_agents_total",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_support_agents_ready_on_attempts": _mean(
                "recovery_tube_interaction_support_agents_ready",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_retained_roots_on_attempts": _mean(
                "recovery_tube_interaction_support_retained_roots",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_eligible_profiles_on_attempts": _mean(
                "recovery_tube_interaction_support_eligible_profiles",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_hypotheses_on_attempts": _mean(
                "recovery_tube_interaction_hypotheses_evaluated",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_noop_hypotheses_on_attempts": _mean(
                "recovery_tube_interaction_noop_hypotheses_skipped",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_no_blocker_rejects_on_attempts": _mean(
                "recovery_tube_interaction_no_blocker_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_unsupported_blocker_rejects_on_attempts": _mean(
                "recovery_tube_interaction_unsupported_blocker_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_residual_physical_rejects_on_attempts": _mean(
                "recovery_tube_interaction_residual_physical_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_root_unrecoverable_rejects_on_attempts": _mean(
                "recovery_tube_interaction_root_unrecoverable_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_joint_incompatibility_rejects_on_attempts": _mean(
                "recovery_tube_interaction_joint_incompatibility_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_environment_compatibility_checks_on_attempts": _mean(
                "recovery_tube_interaction_environment_compatibility_checks",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_environment_compatibility_rejects_on_attempts": _mean(
            ...
            "mean_recovery_tube_interaction_joint_assignment_backtracks_on_attempts": _mean(
                "recovery_tube_interaction_joint_assignment_backtracks",
                mask=np.asarray([bool(r.get("recovery_tube_interaction_response_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_selected_blockers_on_interaction_steps": _mean(
                "recovery_tube_interaction_selected_blocker_count",
                mask=np.asarray([bool(r.get("recovery_tube_selected_is_interaction_response", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_selected_roots_on_interaction_steps": _mean(
                "recovery_tube_interaction_selected_root_count",
                mask=np.asarray([bool(r.get("recovery_tube_selected_is_interaction_response", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_selected_minimum_root_mass_on_interaction_steps": _mean(
                "recovery_tube_interaction_selected_minimum_root_mass",
                mask=np.asarray([bool(r.get("recovery_tube_selected_is_interaction_response", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_selected_maximum_response_burden_on_interaction_steps": _mean(
                "recovery_tube_interaction_selected_maximum_response_burden",
                mask=np.asarray([bool(r.get("recovery_tube_selected_is_interaction_response", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_selected_profile_evaluations_on_interaction_steps": _mean(
                "recovery_tube_interaction_selected_profile_evaluations",
                mask=np.asarray([bool(r.get("recovery_tube_selected_is_interaction_response", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_interaction_selected_environment_agents_on_interaction_steps": _mean(
                "recovery_tube_interaction_selected_environment_agent_count",
                mask=np.asarray([bool(r.get("recovery_tube_selected_is_interaction_response", False)) for r in rows], dtype=bool),
            ) if n else None,
            ...
            "mean_recovery_tube_blocker_query_rcrso_attempts_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_attempts",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_proposals_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_proposals",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_profile_evaluations_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_profile_evaluations",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_verified_profiles_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_verified_profiles",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_selected_roots_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_selected_roots",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_burden_rejects_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_burden_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_roadgraph_or_waymax_kinematic_rejects_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_roadgraph_or_waymax_kinematic_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_ego_current_rejects_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_ego_current_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_ego_shift_rejects_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_ego_shift_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_environment_rejects_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_environment_rejects",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_environment_cache_hits_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_environment_cache_hits",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "recovery_tube_blocker_query_rcrso_selection_rate_on_certified_steps": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_selected",
                mask=np.asarray([bool(r.get("recovery_tube_selected", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_joint_assignment_profiles_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_joint_assignment_profiles",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_certified_hypotheses_on_attempts": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_certified_hypotheses",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_attempted", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_blocker_query_rcrso_selected_assignment_profiles_on_rcrso_steps": _mean(
                "recovery_tube_blocker_conditioned_query_rcrso_selected_assignment_profiles",
                mask=np.asarray([bool(r.get("recovery_tube_blocker_conditioned_query_rcrso_selected", False)) for r in rows], dtype=bool),
            ) if n else None,
            ...
            "mean_recovery_tube_hypotheses_generated_on_probes": _mean(
                "recovery_tube_hypotheses_generated",
                mask=np.asarray([bool(r.get("recovery_tube_probe_used", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_unique_action_hypotheses_on_probes": _mean(
                "recovery_tube_unique_action_hypotheses",
                mask=np.asarray([bool(r.get("recovery_tube_probe_used", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_full_physically_safe_on_probes": _mean(
                "recovery_tube_full_physically_safe",
                mask=np.asarray([bool(r.get("recovery_tube_probe_used", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_shift_closed_on_probes": _mean(
                "recovery_tube_shift_closed",
                mask=np.asarray([bool(r.get("recovery_tube_probe_used", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_nominal_shift_closed_on_probes": _mean(
                "recovery_tube_nominal_shift_closed",
                mask=np.asarray([bool(r.get("recovery_tube_probe_used", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_lower_envelope_shift_closed_on_probes": _mean(
                "recovery_tube_lower_envelope_shift_closed",
                mask=np.asarray([bool(r.get("recovery_tube_probe_used", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_upper_envelope_shift_closed_on_probes": _mean(
            ...
            "mean_recovery_tube_selected_collision_margin_on_certified_steps": _mean(
                "recovery_tube_selected_collision_min_margin_m",
                mask=np.asarray([bool(r.get("recovery_tube_selected", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_selected_shift_collision_margin_on_certified_steps": _mean(
                "recovery_tube_selected_shift_collision_min_margin_m",
                mask=np.asarray([bool(r.get("recovery_tube_selected", False)) for r in rows], dtype=bool),
            ) if n else None,
            "mean_recovery_tube_selected_fallback_score_delta_on_action_changes": _mean(
                "recovery_tube_selected_fallback_score_delta",
                mask=np.asarray([bool(r.get("recovery_tube_action_changed", False)) for r in rows], dtype=bool),
            ) if n else None,
