...
def _failure_localization(p:dict)->dict:
    ...
    for label,key in mapping.items():
        ...
        for r in pos:
            ...
            v=d.get(f'fallback_rate_before_first_{label.lower()}')
            if v is not None: vals.append(float(v))
            a=d.get(f'fallback_at_action_before_first_{label.lower()}')
            if a is not None: at.append(bool(a))
        out[label]={'positive_episodes':len(pos),'mean_fallback_rate_before_first_event':float(np.mean(vals)) if vals else None,'fallback_action_immediately_before_first_event_rate':float(np.mean(at)) if at else None}
    return out
...
def main():
    ...
    out={'schema_version':'cowp_v16_8_26_waymax_physical_compare_v1','cowp_failure_localization':_failure_localization(base)}
