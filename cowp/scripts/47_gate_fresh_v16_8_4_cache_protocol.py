...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Strict preflight preventing stale v16.8 overlays from being used as v16.8.4 BCS-RMR-BCTE data."
    )
    ...
    manifest_path = cowp_root / "data_manifest_v16_8_4.json"
    ...
    if manifest is None:
        reasons.append("missing or unreadable data_manifest_v16_8_4.json")
    else:
        if manifest.get("schema_version") != "cowp_v16_8_4_bcs_rmr_bcte_data_v1":
    ...
    for split_name, split in (("train", train), ("val", val)):
        ...
        if not meta.get("exists") or meta.get("read_error"):
            ...
            if str(meta.get("sidecar_subdir", "")) != ".transport_v16_8_4":
                reasons.append(f"{split_name}: sidecar_subdir={meta.get('sidecar_subdir')!r}, expected '.transport_v16_8_4'")
    ...
    report = {
        "schema_version": "cowp_v16_8_4_fresh_cache_protocol_gate_v1",
        "pass": not reasons,
        ...
        "reasons": reasons,
        "interpretation": (
            "Fresh v16.8.4 cache identity/provenance passed; training may use these caches."
            if not reasons else
            "Do not train v16.8.4 on these caches. In particular, a transport overlay cannot retrofit a new ego proposal bank into stale raw cache files."
        ),
    }
