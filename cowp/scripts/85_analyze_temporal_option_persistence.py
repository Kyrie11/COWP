...
def _bootstrap_mean_ci(x: np.ndarray, seed: int = 16832, draws: int = 10000) -> list[float] | None:
...
def main() -> None:
    ...
    out: dict[str, Any] = {
        "schema_version": "cowp_temporal_option_persistence_analysis_v1",
    ...
    if "rvr" in rowmap:
        ...
        for name in ("sov", "bihorizon", "trihorizon", "commitment"):
    ...
    for ref in ("sov", "bihorizon"):
        if ref in rowmap:
            out[f"paired_vs_{ref}"] = {}
            for name in ("trihorizon", "commitment"):
                if name in rowmap:
