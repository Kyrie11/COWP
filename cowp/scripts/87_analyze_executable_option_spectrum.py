...
def main() -> None:
    ...
    ap.add_argument("--transition-guarded")
    ap.add_argument("--executable-spectrum")
    ...
    data = {
        ...
        "transition_guarded": _load(args.transition_guarded),
        "executable_spectrum": _load(args.executable_spectrum),
    }
    ...
    out: dict[str, Any] = {
        "schema_version": "cowp_executable_option_spectrum_analysis_v1",
    ...
    candidates = [n for n in ("transition_guarded", "executable_spectrum") if n in rowmap]
    ...
    if "rvr" in rowmap:
        ...
        for name in [n for n in ("v33_rosh", *candidates) if n in rowmap]:
            rows = rowmap[name]
    ...
    if "v33_rosh" in rowmap:
        ...
        # Diagnostic only: v16.8.33 failed promotion solely because it induced two
        # kinematics events vs COWP.  Do not make these selected counterexamples a
        # promotion threshold; expose them so the mechanism can be falsified.
    ...
    if len(candidates) == 2:
        out["paired_executable_vs_transition_guarded"] = _paired(
            rowmap["transition_guarded"], rowmap["executable_spectrum"], ids
        )

    # Keep the exact v16.8.33 promotion gate unchanged.  This avoids moving the
