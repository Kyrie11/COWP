...
def main():
    ...
    evaluated_threshold=float(main.get('witness_threshold', float('nan')) or float('nan'))
    if args.calibration_json:
        cal=json.load(open(args.calibration_json))
        selected=cal.get('selection_metrics', {})
        if isinstance(selected, dict) and selected:
            main=selected
            evaluated_threshold=float(cal.get('witness_threshold', selected.get('witness_threshold', evaluated_threshold)))
    ...
    report={
        ...
        'evaluated_witness_threshold':evaluated_threshold,
    ...
    report['pass']=bool(
        ...
        and report['witness_auprc_pass'] and report['bcot_auprc_pass'] and report['accepted_rate_pass']
