...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v10_probe100_seed2026}"
...
TRANSPORT_EPOCHS="${TRANSPORT_EPOCHS:-16}"
...
cp configs/label_cowp_v10.yaml "$OUT_ROOT/configs/label_cowp_v10.yaml"
cp configs/label_cowp_v10_pareto_ablation.yaml "$OUT_ROOT/configs/label_cowp_v10_pareto_ablation.yaml"
cp configs/train_cowp_v10.yaml "$OUT_ROOT/configs/train_cowp_v10.yaml"
cp configs/eval_cowp_v10.yaml "$OUT_ROOT/configs/eval_cowp_v10.yaml"
sed -i -E "0,/^  seed:/{s/^  seed:.*/  seed: ${TRAIN_SEED}/}" "$OUT_ROOT/configs/train_cowp_v10.yaml"
LABEL_CFG="$OUT_ROOT/configs/label_cowp_v10.yaml"
PARETO_CFG="$OUT_ROOT/configs/label_cowp_v10_pareto_ablation.yaml"
TRAIN_CFG="$OUT_ROOT/configs/train_cowp_v10.yaml"
EVAL_CFG="$OUT_ROOT/configs/eval_cowp_v10.yaml"

if [[ "$DETACH" == "1" && "${COWP_V10_DETACHED:-0}" != "1" ]]; then
  export COWP_V10_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v10] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
if [[ "$RUN_OFFLINE" == "1" ]]; then
  ...
  logrun verify_mechanism "$PYTHON_BIN" -u -m cowp.scripts.25_verify_mechanism_effect \
    ...
    --min-witness-auprc 0.55 --min-accepted-rate 0.10 --max-fallback 0.25 \
...
echo "[cowp_v10] complete: $OUT_ROOT"
