...
export OUT_ROOT="${OUT_ROOT:-outputs/cowp_v16_3_pipeline_v9labels_seed2026}"
...
if [[ "$BACKGROUND" == "1" && "${COWP_V16_3_BACKGROUND_CHILD:-0}" != "1" ]]; then
  export COWP_V16_3_BACKGROUND_CHILD=1
  nohup env BACKGROUND=0 bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 < /dev/null &
  pid=$!
  printf '%s\n' "$pid" > "$OUT_ROOT/logs/driver.pid"
  echo "[cowp_v16.2] background pid=$pid"
  echo "[cowp_v16.2] log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
bash run_cowp_v16_3_dual_gpu.sh
