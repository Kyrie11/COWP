...
# v16.8.27 repair-only: NO dataset/label/cache rebuild and NO model retraining.
# Purpose: restore strict-Waymax conventional-safe semantics, then rerun the
# exact same 200-ID physical attribution before any new algorithm is designed.
MODE="${1:-help}"
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_27_conventional_integrity}"
...
PY
  ...
  help|*)
    cat <<EOF
v16.8.27 is repair-only. No dataset rebuild, no cache rebuild, no training.
Recommended order:
  $0 sanity
...
strict attribution has been inspected.
EOF
    ;;
esac
