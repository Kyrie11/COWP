...
export PROBE_ROOT="${PROBE_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_13_support_strict_probe}"
...
FRESH="$PROBE_ROOT/labels_val_v16_8_13"; PROFILE="$PROBE_ROOT/fresh_probe_profile.jsonl"; SUMMARY="$PROBE_ROOT/fresh_probe_profile_summary.json"
...
BASE_VERDICT="$PROBE_ROOT/base_screen_verdict.json"; VERDICT="$PROBE_ROOT/v16_8_13_strict_verdict.json"; FP="$PROBE_ROOT/v16_8_13_code_fingerprint.sha256"
...
run_semantic model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support --cache-dir "$FRESH" --sample-scenes 0 --min-class-examples 32 --min-source-examples 32 --max-unauditable-critical-rate 0.01 --min-certificate-complete-scene-rate 0.98 --output "$MODEL_SUPPORT" --strict; MS_RC=$?
