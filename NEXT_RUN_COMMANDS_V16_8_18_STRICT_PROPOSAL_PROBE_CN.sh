...
payload={'schema_version':'cowp_v16_8_20_strict_composite_verdict_v5','strict':True,'code_fingerprint_sha256':fp,'pass':passed,'recommend_full_rebuild':passed,'checks':checks,'failed_checks':failed,'sparse_build_integrity':sparse,'semantic_returncodes':{'supervision':int(sup_rc),'model_support':int(ms_rc),'screen':int(screen_rc)},'base_screen':base,'model_support_coverage':{'statistics':ms.get('coverage_statistics',{}),'by_stratum':ms.get('coverage_by_stratum',{})},'natural_support':{k:nat.get(k) for k in ('critical_agents_selected','mechanism_auditable_critical_agents','mechanism_unauditable_critical_agents','mechanism_unauditable_rate','mechanism_unauditable_future_support','mechanism_unauditable_finalizer_counts','mechanism_unauditable_with_sufficient_future_but_no_substantial_route_geometry','mechanism_unauditable_with_insufficient_future','rootless_critical_agents','critical_agents_with_lt2_low_burden_roots','protected_auditable_critical_agents','protected_without_prio_root','protected_prio_root_coverage','empirical_corridor_roots','rootless_dominant_rejection','lt2_low_burden_dominant_rejection','priority_rejection_reason_counts','map_rejected_min_distance_summary_m','map_rejected_max_distance_summary_m')},'next_action':'Run train-pilot with the identical fingerprint before full-core.' if passed else ('Do not full-rebuild; failed checks: '+', '.join(failed)+'. Inspect base_screen first when proposal_causal_screen_pass is false; otherwise inspect the named supervision/model/natural check.')}
