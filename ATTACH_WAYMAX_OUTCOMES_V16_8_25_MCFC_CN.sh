...
# Optimized three-split Waymax candidate replay for v16.8.25-MCFC.
...
REPLAY_ROOT="${REPLAY_ROOT:-$COWP_ROOT/waymax_replay_v16_8_25_mcfc}"
...
WAYMAX_JAX_COMPILATION_CACHE_DIR="${WAYMAX_JAX_COMPILATION_CACHE_DIR:-$COWP_ROOT/.jax_compilation_cache_waymax_v16_8_25_mcfc}"
  ...
  for ((s=0;s<NUM_SHARDS;s++)); do
    ...
    (
      ...
      "$PYTHON_BIN" -u -m cowp.scripts.13_replay_waymax_candidates \
        ...
        --label-config configs/label_cowp_v16_8_25_mcfc.yaml \
...
"$PYTHON_BIN" - "$OUT_TRAIN" "$OUT_VAL" "$OUT_TEST" "$COWP_ROOT/waymax_outcome_support_v16_8_25_mcfc.json" <<'PY'
import json,sys
...
payload={'schema_version':'cowp_v16_8_25_mcfc_waymax_outcome_support_v1','splits':reports,
