...
FRESH_LABELS="$SMOKE_ROOT/labels_val_v16_8_16"
...
BASE_SCREEN="$SMOKE_ROOT/base_screen_verdict.json"; VERDICT="$SMOKE_ROOT/v16_8_16_smoke_verdict.json"; FP_FILE="$SMOKE_ROOT/v16_8_16_code_fingerprint.sha256"
...
# v16.8.16: derive the stress manifest from the CURRENT old validation cache.
...
run_semantic model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support --cache-dir "$FRESH_LABELS" --sample-scenes 0 --min-class-examples 8 --min-source-examples 8 --max-unauditable-critical-rate 0.01 --min-certificate-complete-scene-rate 0.95 --min-protected-prio-coverage 0.95 --output "$MODEL_SUPPORT" --strict; MS_RC=$?
...
payload={
 'schema_version':'cowp_v16_8_16_smoke_composite_verdict_v3', 'code_fingerprint_sha256':fp,
