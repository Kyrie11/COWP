...
COWP_ROOT="${COWP_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_2_bcte}"
...
TRANSPORT_TRAIN="$COWP_ROOT/tensor_cache_train_waymax_transport_v16_8_2"
TRANSPORT_VAL="$COWP_ROOT/tensor_cache_val_waymax_transport_v16_8_2"
REPLAY_DIR="$COWP_ROOT/waymax_replay_v16_8_2"
...
run augment_train "$PYTHON_BIN" -u -m cowp.scripts.26_augment_transport_labels \
  ...
  --sidecar-subdir .transport_v16_8_2 --force
...
cat > "$COWP_ROOT/data_manifest_v16_8_2.json" <<JSON
{
  "schema_version": "cowp_v16_8_2_bcte_data_v1",
  ...
  "notes": "Fresh labels include BCTE interaction-timing proposals, explicit root-conditioned response search, transported OPR, hard protected-relation semantics, and map-screened proposals."
}
JSON

echo "[v16.8.2 BCTE data] complete: $COWP_ROOT"
