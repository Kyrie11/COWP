...
A fixed endpoint ball is not an adequate definition of root identity: it treats a low-probability exploratory root and a probability-carrying root equally, ignores large intermediate excursions that return near the endpoint, and can suppress a valid observational maneuver. Because OPR is defined on retained probability mass, COWP instead uses a probability-mass-aware, multi-horizon semantic envelope. Let $\bar p_{im}$ be the predicted natural-root probability and $r_{s(m)}(t)$ a source-conditioned soft envelope. We regularize
\begin{equation}
    \mathcal{L}_{\mathrm{id}}
    =
    \sum_{i,m}\bar p_{im}
    \left[
      \max_t\frac{\|\delta p_{im}(t)\|_2}{r_{s(m)}(t)}-\eta
    \right]_+^2,
    \label{eq:natural_mass_envelope}
\end{equation}
where $\eta<1$ defines an interior margin. The root probabilities are detached in this term so the decoder cannot avoid geometric regularization merely by suppressing the corresponding logit. Separately, every mode is constrained by a wider emergency envelope $\bar r_{s(m)}(t)>r_{s(m)}(t)$. If a raw integrated residual exceeds this envelope, the complete control sequence is scaled and re-integrated. Thus all modes remain physically bounded, while the semantic constraint is concentrated on the probability mass that enters OPR.
