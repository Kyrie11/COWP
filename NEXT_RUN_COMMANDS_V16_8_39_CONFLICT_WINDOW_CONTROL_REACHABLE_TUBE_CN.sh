...
# Preregistered branch after v16.8.38 showed that certified controller-lifted
# support is real but sparse. Keep the unchanged full physical/shift-closure
# certificates and expand only the causal longitudinal control-sequence family
# with event-derived endpoint release through the nominal conflict window.
#
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_39_conflict_window_control_reachable_tube}"
...
for method in [x.strip() for x in methods.split(',') if x.strip()]:
    if method!='cowp_conflict_window_control_reachable_tube':
        raise SystemExit(f'ERROR: {method} is not a v16.8.39 promotion candidate')
    g=gates.get('conflict_window_control_reachable_tube')
...
analyze_set() {
  ...
  local args=(--cowp "$cowp" --rvr "$rvr" --stage "$stage" --development-selected \
    --conflict-window-control-reachable-tube "$OUT_ROOT/${tag}_cowp_conflict_window_control_reachable_tube_merged.json" \
    --output "$OUT_ROOT/${tag}_conflict_window_control_reachable_tube_analysis.json")
  ...
  python -m cowp.scripts.92_analyze_conflict_window_control_reachable_tube "${args[@]}"
}
...
case "$MODE" in
  sanity)
    python -m py_compile cowp/waymax_eval/policy_wrapper.py cowp/waymax_eval/rollout.py cowp/waymax_eval/metrics_cowp.py \
      ...
      cowp/scripts/92_analyze_conflict_window_control_reachable_tube.py
    python -m pytest -q \
    ...
    ensure_ids
    echo "v16.8.39 focused semantic/integrity sanity passed"
    ;;
  make_ids) ensure_ids ;;
  build_tfindex)
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v39" "$CF48_IDS" "${METHODS:-cowp_conflict_window_control_reachable_tube}" ;;
  analyze_counterfactual48)
    analyze_set "counterfactual48" "counterfactual48_v39" "$REF_CF48_COWP" "$REF_CF48_RVR" ;;
  fresh37_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_conflict_window_control_reachable_tube}"
    require_gate_pass "$OUT_ROOT/counterfactual48_v39_conflict_window_control_reachable_tube_analysis.json" "$promoted" "counterfactual48"
    run_parallel_methods "fresh37_v39" "$FRESH37_IDS" "$promoted" ;;
  analyze_fresh37)
    analyze_set "fresh37" "fresh37_v39" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR" ;;
  confirm200_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_conflict_window_control_reachable_tube}"
    require_gate_pass "$OUT_ROOT/fresh37_v39_conflict_window_control_reachable_tube_analysis.json" "$promoted" "fresh37"
...
STOP here unless preregistered_gate.conflict_window_control_reachable_tube.pass == true.
The six Stage-1 GO conditions are unchanged from v16.8.33--38.

Only after Stage-1 pass:
  PROMOTED_METHODS=cowp_conflict_window_control_reachable_tube $0 fresh37_parallel2
  $0 analyze_fresh37

Only after fresh37 pass:
  PROMOTED_METHODS=cowp_conflict_window_control_reachable_tube $0 confirm200_parallel2
