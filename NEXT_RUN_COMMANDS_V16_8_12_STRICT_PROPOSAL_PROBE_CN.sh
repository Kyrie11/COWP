...
export PROBE_ROOT="${PROBE_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_12_support_strict_probe}"
...
FRESH="$PROBE_ROOT/labels_val_v16_8_12"; PROFILE="$PROBE_ROOT/fresh_probe_profile.jsonl"; SUMMARY="$PROBE_ROOT/fresh_probe_profile_summary.json"
...
BASE_VERDICT="$PROBE_ROOT/base_screen_verdict.json"; VERDICT="$PROBE_ROOT/v16_8_12_strict_verdict.json"; FP="$PROBE_ROOT/v16_8_12_code_fingerprint.sha256"
...
run_semantic model_support "$PYTHON_BIN" -m cowp.scripts.65_audit_model_support --cache-dir "$FRESH" --sample-scenes 0 --min-class-examples 32 --min-source-examples 32 --output "$MODEL_SUPPORT" --strict; MS_RC=$?
...
checks={'proposal_causal_screen_pass':int(screen_rc)==0,'training_supervision_pass':bool(sup.get('pass',False)),'model_support_pass':bool(ms.get('pass',False)),'natural_rootless_zero':int(nat.get('rootless_critical_agents',-1))==0,'natural_lt2_low_burden_zero':int(nat.get('critical_agents_with_lt2_low_burden_roots',-1))==0}
passed=all(checks.values())
payload={'schema_version':'cowp_v16_8_12_strict_composite_verdict_v1','strict':True,'code_fingerprint_sha256':fp,'pass':passed,'recommend_full_rebuild':passed,'checks':checks,'semantic_returncodes':{'supervision':int(sup_rc),'model_support':int(ms_rc),'screen':int(screen_rc)},'base_screen':base,'natural_support':{k:nat.get(k) for k in ('critical_agents','rootless_critical_agents','critical_agents_with_lt2_low_burden_roots','rootless_dominant_rejection','lt2_low_burden_dominant_rejection','priority_rejection_reason_counts')},'next_action':'Run train-pilot with the identical fingerprint before full-core.' if passed else 'Do not full-rebuild; inspect strict natural/model-support diagnostics.'}
