...
# v16.8.6 paper-grade rebuild for BCS-RMR-BCTE + Priority-Commitment Hold-Release proposals.
...
COWP_ROOT="${COWP_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v16_8_6_priority_commitment}"
...
if [[ -s "$FINGERPRINT_FILE" ]]; then
  ...
  echo "existing labels without a v16.8.6 fingerprint; use a new COWP_ROOT or set ALLOW_UNVERIFIED_RESUME=1 after manual audit" >&2
  exit 3
else
