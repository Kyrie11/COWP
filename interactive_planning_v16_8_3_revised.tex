...
\end{equation}
where $\tau^k_e$ is a continuous ego trajectory and $a^k_e$ is a proposal descriptor. In the evaluated implementation, the bank is a finite set of deterministic kinematic primitives: constant-acceleration, comfort-bounded stop/creep, conflict-timing, terminal-state, and delayed lateral-offset trajectories. Interaction timing is covered by a robust multi-region bidirectional conflict-time envelope that proposes ego passage before the earliest plausible or after the latest plausible arrival of a nearby agent at each reachable conflict region. Synthetic primitives are screened for acceleration, deceleration, a prefix-robust jerk statistic, duplicate endpoints, non-negative terminal speed at a requested arrival, and sampled lane-corridor compliance. Each candidate stores its proposal source, conflict region, timing side, target agent, target time, gap, and solved acceleration. The macro label and provenance describe how a primitive was generated; neither is itself evidence of route feasibility, right-of-way, successful negotiation, or non-coercive feasibility.
...
The evaluated proposal bank contains: logged ego replay; constant-acceleration longitudinal primitives; smooth stop and creep primitives; conflict-region stopping margins; legacy ego-only conflict-arrival offsets; delayed lateral-offset primitives; and terminal speed/position variants.  We further introduce a \emph{Robust Multi-Region Bidirectional Conflict-Time Envelope} (RMR-BCTE) proposal family.  The generator ranks forward-reachable conflict regions by nominal ego time-to-arrival and retains the first $R$.  For agent $j$ approaching region $r$, a bounded-acceleration timing model produces an arrival envelope
\begin{equation}
    [t^-_{jr},t^+_{jr}],
\end{equation}
which is clipped to a proposal-level uncertainty spread rather than interpreted as a formal reachable-set guarantee.  RMR-BCTE proposes the ego target times
\begin{equation}
    t^{\mathrm{before}}_{ejr}=t^-_{jr}-\Delta,
    \qquad
    t^{\mathrm{after}}_{ejr}=t^+_{jr}+\Delta,
\end{equation}
and solves the bounded constant-acceleration primitive
\begin{equation}
    a_e(d_r,t)=\frac{2(d_r-v_{e,0}t)}{t^2},
    \qquad
    a_{\min}\le a_e(d_r,t)\le a_{\max},
    \qquad
    v_{e,0}+a_e t\ge 0.
\end{equation}
The final condition prevents a clipped zero-speed trajectory from being mislabeled as arriving after it has already stopped.  Candidate quotas and acceleration-space deduplication preserve room for lane-change, stop, creep, and terminal alternatives.  The offline and online generators use the same before/after semantics, while candidate provenance makes source-specific coverage and NCF yield auditable.  RMR-BCTE changes only proposal coverage: every generated trajectory remains subject to conventional safety and the root-conditioned non-coercive certificate, so adding proposals cannot turn an uncertified maneuver into a certified one.
