...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v14_temporal_riot_probe100_seed2026}"
...
cp configs/label_cowp_v14.yaml "$OUT_ROOT/configs/label_cowp_v14.yaml"
...
cp configs/train_cowp_v14.yaml "$OUT_ROOT/configs/train_cowp_v14.yaml"
cp configs/eval_cowp_v14.yaml "$OUT_ROOT/configs/eval_cowp_v14.yaml"
sed -i -E "0,/^  seed:/{s/^  seed:.*/  seed: ${TRAIN_SEED}/}" "$OUT_ROOT/configs/train_cowp_v14.yaml"
LABEL_CFG="$OUT_ROOT/configs/label_cowp_v14.yaml"
...
TRAIN_CFG="$OUT_ROOT/configs/train_cowp_v14.yaml"
EVAL_CFG="$OUT_ROOT/configs/eval_cowp_v14.yaml"

if [[ "$DETACH" == "1" && "${COWP_V14_DETACHED:-0}" != "1" ]]; then
  export COWP_V14_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v14] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
logrun gate_natural_basis "$PYTHON_BIN" -u -m cowp.scripts.32_gate_natural_basis \
  ...
  --max-source-ce 0.30 --max-priority-bce 0.45 \
  --max-neutral-consistency-m 10.0 --max-typed-untyped-gap-m 4.0
if [[ "$STOP_AFTER_STAGE" == "natural" ]]; then
  echo "[cowp_v14] stopped after natural gate: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "transport" ]]; then
  echo "[cowp_v14] stopped after transport: $OUT_ROOT"
  exit 0
fi
...
[[ -s "$CKPT" ]] || { echo "No v14 planner checkpoint. Set CKPT or enable RUN_PLANNER." >&2; exit 2; }
echo "[checkpoint] $CKPT"
if [[ "$STOP_AFTER_STAGE" == "planner" ]]; then
  echo "[cowp_v14] stopped after planner: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "offline" ]]; then
  echo "[cowp_v14] stopped after learned-offline verification: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "probe" ]]; then
  echo "[cowp_v14] stopped after probe: $OUT_ROOT"
  exit 0
fi
...
echo "[cowp_v14] complete: $OUT_ROOT"
