...
# v16.8.36: Control-Projected Semantic Recovery Frontier (CPRF)
# One-factor question after v16.8.35: is the remaining bottleneck the binary
# COWP-vs-global-RVR endpoint restriction rather than the option-set observable?
#
...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_36_control_projected_recovery_frontier}"
...
for method in [x.strip() for x in methods.split(',') if x.strip()]:
    if method!='cowp_control_projected_recovery_frontier':
        raise SystemExit(f'ERROR: {method} is not a v16.8.36 promotion candidate')
    g=gates.get('control_projected_recovery_frontier')
...
analyze_set() {
  ...
  local args=(--cowp "$cowp" --rvr "$rvr" --stage "$stage" --development-selected \
    --control-projected-recovery-frontier "$OUT_ROOT/${tag}_cowp_control_projected_recovery_frontier_merged.json" \
    --output "$OUT_ROOT/${tag}_control_projected_recovery_frontier_analysis.json")
  if [[ "$stage" == "counterfactual48" ]]; then
    args+=(--v33-rosh "$REF_CF48_V33_ROSH" --v35-control-projected-spectrum "$REF_CF48_V35_CPOSH")
  fi
  python -m cowp.scripts.89_analyze_control_projected_recovery_frontier "${args[@]}"
}
...
case "$MODE" in
  sanity)
    python -m py_compile cowp/waymax_eval/policy_wrapper.py cowp/waymax_eval/rollout.py cowp/waymax_eval/metrics_cowp.py \
      ...
      cowp/scripts/89_analyze_control_projected_recovery_frontier.py
    python -m pytest -q \
    ...
    ensure_ids
    echo "v16.8.36 focused semantic/integrity sanity passed"
    ;;
  make_ids) ensure_ids ;;
  build_tfindex)
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v36" "$CF48_IDS" "${METHODS:-cowp_control_projected_recovery_frontier}" ;;
  analyze_counterfactual48)
    analyze_set "counterfactual48" "counterfactual48_v36" "$REF_CF48_COWP" "$REF_CF48_RVR" ;;
  fresh37_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_control_projected_recovery_frontier}"
    require_gate_pass "$OUT_ROOT/counterfactual48_v36_control_projected_recovery_frontier_analysis.json" "$promoted" "counterfactual48"
    run_parallel_methods "fresh37_v36" "$FRESH37_IDS" "$promoted" ;;
  analyze_fresh37)
    analyze_set "fresh37" "fresh37_v36" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR" ;;
  confirm200_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_control_projected_recovery_frontier}"
    require_gate_pass "$OUT_ROOT/fresh37_v36_control_projected_recovery_frontier_analysis.json" "$promoted" "fresh37"
...
STOP here unless preregistered_gate.control_projected_recovery_frontier.pass == true.
The six Stage-1 GO conditions are unchanged from v16.8.33--35.

Only after Stage-1 pass:
  PROMOTED_METHODS=cowp_control_projected_recovery_frontier $0 fresh37_parallel2
  $0 analyze_fresh37

Only after fresh37 pass:
  PROMOTED_METHODS=cowp_control_projected_recovery_frontier $0 confirm200_parallel2
  $0 analyze_confirm200

Do NOT tune thresholds/weights after a failed gate. Do NOT expand the proposal bank in this version.
EOF
    ;;
esac
