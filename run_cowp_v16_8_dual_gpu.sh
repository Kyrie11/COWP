...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v16_8_2_certificate_consistent_v9base_seed2026}"
...
VAL_CACHE="${VAL_CACHE:-/data0/senzeyu2/dataset/COWP/formal/tensor_cache_val_waymax_transport_v16_8}"
...
AUG_SIDECAR_SUBDIR="${AUG_SIDECAR_SUBDIR:-.transport_v16_8}"
NATURAL_EPOCHS="${NATURAL_EPOCHS:-20}"
TRANSPORT_EPOCHS="${TRANSPORT_EPOCHS:-24}"
PLANNER_EPOCHS="${PLANNER_EPOCHS:-16}"
...
BCOT_BUDGET_SWEEP="${BCOT_BUDGET_SWEEP:-0.05,0.08,0.10,0.12,0.15,0.18,0.22,0.26,0.30,0.35,0.40,0.45,0.50,0.55,0.60,0.65,0.70,0.75,0.80,0.85,0.90,0.95,0.98}"
...
DATA_PROTOCOL="${DATA_PROTOCOL:-v16_8_root_conditioned_overlay}"
...
"$PYTHON_BIN" -u -m cowp.scripts.42_write_run_provenance \
  ...
  --file "configs/model_cowp_v16_8.yaml=$CONFIG_CANDIDATE_DIR/model_cowp_v16_8.yaml" \
  --file "configs/label_cowp_v16_8.yaml=$CONFIG_CANDIDATE_DIR/label_cowp_v16_8.yaml" \
  --file "configs/train_cowp_v16_8.yaml=$CONFIG_CANDIDATE_DIR/train_cowp_v16_8.yaml" \
  --file "configs/eval_cowp_v16_8.yaml=$CONFIG_CANDIDATE_DIR/eval_cowp_v16_8.yaml" \
...
cp "$CONFIG_CANDIDATE_DIR/model_cowp_v16_8.yaml" "$OUT_ROOT/configs/model_cowp_v16_8.yaml"
cp "$CONFIG_CANDIDATE_DIR/label_cowp_v16_8.yaml" "$OUT_ROOT/configs/label_cowp_v16_8.yaml"
cp "$CONFIG_CANDIDATE_DIR/label_cowp_v16_8_pareto_ablation.yaml" "$OUT_ROOT/configs/label_cowp_v16_8_pareto_ablation.yaml"
cp "$CONFIG_CANDIDATE_DIR/label_cowp_v16_8_pairmax_ablation.yaml" "$OUT_ROOT/configs/label_cowp_v16_8_pairmax_ablation.yaml"
cp "$CONFIG_CANDIDATE_DIR/train_cowp_v16_8.yaml" "$OUT_ROOT/configs/train_cowp_v16_8.yaml"
cp "$CONFIG_CANDIDATE_DIR/eval_cowp_v16_8.yaml" "$OUT_ROOT/configs/eval_cowp_v16_8.yaml"
...
MODEL_CFG="$OUT_ROOT/configs/model_cowp_v16_8.yaml"
LABEL_CFG="$OUT_ROOT/configs/label_cowp_v16_8.yaml"
PARETO_CFG="$OUT_ROOT/configs/label_cowp_v16_8_pareto_ablation.yaml"
PAIRMAX_CFG="$OUT_ROOT/configs/label_cowp_v16_8_pairmax_ablation.yaml"
TRAIN_CFG="$OUT_ROOT/configs/train_cowp_v16_8.yaml"
EVAL_CFG="$OUT_ROOT/configs/eval_cowp_v16_8.yaml"

if [[ "$DETACH" == "1" && "${COWP_V16_8_DETACHED:-0}" != "1" ]]; then
  export COWP_V16_8_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v16.8] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "diagnose" ]]; then
  echo "[cowp_v16.8] stopped after diagnostics: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "natural" ]]; then
  echo "[cowp_v16.8] stopped after natural gate: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "transport" ]]; then
  echo "[cowp_v16.8] stopped after transport: $OUT_ROOT"
  exit 0
fi
...
[[ -s "$CKPT" ]] || { echo "No v16.8 planner checkpoint. Set CKPT or enable RUN_PLANNER." >&2; exit 2; }
echo "[checkpoint] $CKPT"
if [[ "$STOP_AFTER_STAGE" == "planner" ]]; then
  echo "[cowp_v16.8] stopped after planner: $OUT_ROOT"
  exit 0
fi
...
if [[ "$RUN_OFFLINE" == "1" ]]; then
  ...
  logrun calibrate_bcot "$PYTHON_BIN" -u -m cowp.scripts.31_calibrate_bcot_budget \
    ...
    --min-priority-ncf-recall "${MIN_PRIORITY_NCF_RECALL:-0.30}" \
    --min-priority-ncf-precision "${MIN_PRIORITY_NCF_PRECISION:-0.50}" \
    --max-priority-burden-transfer "${MAX_PRIORITY_BURDEN_TRANSFER:-0.45}" \
    --min-global-ncf-recall "${MIN_GLOBAL_NCF_RECALL:-0.18}" \
    --min-accepted-rate "${MIN_ACCEPTED_RATE:-0.10}" \
    --max-fallback "${MAX_FALLBACK_RATE:-0.25}" \
...
PY2
  ...
  quality_gate verify_mechanism "$PYTHON_BIN" -u -m cowp.scripts.25_verify_mechanism_effect \
    ...
    --min-priority-root-auprc "${MIN_PRIORITY_ROOT_AUPRC:-0.50}" \
    --min-accepted-rate "${MIN_ACCEPTED_RATE:-0.10}" \
    --max-fallback "${MAX_FALLBACK_RATE:-0.25}" \
...
if [[ "$STOP_AFTER_STAGE" == "offline" ]]; then
  echo "[cowp_v16.8] stopped after learned-offline verification: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "probe" ]]; then
  echo "[cowp_v16.8] stopped after probe: $OUT_ROOT"
  exit 0
fi
...
echo "[cowp_v16.8] complete: $OUT_ROOT"
