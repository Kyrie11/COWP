...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v7}"
...
RUN_TOPK_ABLATION="${RUN_TOPK_ABLATION:-1}"
RUN_FULL="${RUN_FULL:-0}"
...
# Planner/certificate/outcome heads are fine-tuned on the v6 representation.
# Keep the shared graph/candidate/witness stack frozen to prevent indirect witness pollution.
...
INIT_CKPT="${INIT_CKPT:-outputs/cowp_v6/checkpoints/planner/cowp_planner_best.pt}"
CKPT="${CKPT:-}"
...
cp configs/label_cowp_v7.yaml "$OUT_ROOT/configs/label_cowp_v7.yaml"
cp configs/label_cowp_v7_topk_ablation.yaml "$OUT_ROOT/configs/label_cowp_v7_topk_ablation.yaml"
cp configs/train_cowp_v7.yaml "$OUT_ROOT/configs/train_cowp_v7.yaml"
LABEL_CFG="$OUT_ROOT/configs/label_cowp_v7.yaml"
TOPK_CFG="$OUT_ROOT/configs/label_cowp_v7_topk_ablation.yaml"
TRAIN_CFG="$OUT_ROOT/configs/train_cowp_v7.yaml"

if [[ "$DETACH" == "1" && "${COWP_V7_DETACHED:-0}" != "1" ]]; then
  export COWP_V7_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v7] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
if [[ "$RUN_TRAIN" == "1" ]]; then
  if best_ckpt >/dev/null && [[ "$FORCE_TRAIN" != "1" ]]; then
    ...
    if [[ -s "$INIT_CKPT" ]]; then
      ...
      echo "[train] warning: INIT_CKPT not found: $INIT_CKPT; training from scratch is not recommended for this head-only v7 stage" >&2
    fi
...
if [[ -z "$CKPT" || ! -s "$CKPT" ]]; then
  echo "No v7 best checkpoint. Set CKPT=... or enable training." >&2
  exit 2
fi
...
if [[ "$RUN_PROBE" == "1" ]]; then
  probe_cowp="$OUT_ROOT/eval/probe/cowp_pareto_${PROBE_SCENARIOS}.json"
  ...
  if ! json_valid "$probe_cowp" || ! json_valid "$probe_conv"; then
    run_online_one cowp "$GPU0" "$PROBE_SCENARIOS" "$probe_cowp" "$OUT_ROOT/logs/probe_cowp_pareto.log" "$LABEL_CFG" & p0=$!
    run_online_one conventional_safety "$GPU1" "$PROBE_SCENARIOS" "$probe_conv" "$OUT_ROOT/logs/probe_conventional.log" "$LABEL_CFG" & p1=$!
    wait "$p0"; wait "$p1"
  fi
  logrun compare_probe "$PYTHON_BIN" -u -m cowp.scripts.23_compare_eval_outputs \
    --a "$probe_conv" --b "$probe_cowp" --output "$OUT_ROOT/eval/probe/compare_conventional_vs_pareto.json"

  if [[ "$RUN_TOPK_ABLATION" == "1" ]]; then
    probe_topk="$OUT_ROOT/eval/probe/cowp_topk_${PROBE_SCENARIOS}.json"
    if ! json_valid "$probe_topk"; then
      run_online_one cowp "$GPU1" "$PROBE_SCENARIOS" "$probe_topk" "$OUT_ROOT/logs/probe_cowp_topk.log" "$TOPK_CFG"
    fi
    logrun compare_frontier_ablation "$PYTHON_BIN" -u -m cowp.scripts.23_compare_eval_outputs \
      --a "$probe_topk" --b "$probe_cowp" --output "$OUT_ROOT/eval/probe/compare_topk_vs_pareto.json"
  fi
fi
...
if [[ "$RUN_FULL" == "1" ]]; then
  ...
  c0="$OUT_ROOT/eval/waymax/cowp_pareto_shard0.json"
  c1="$OUT_ROOT/eval/waymax/cowp_pareto_shard1.json"
  ...
  logrun merge_cowp "$PYTHON_BIN" -u -m cowp.scripts.17_merge_waymax_shards \
    --output "$OUT_ROOT/eval/waymax/cowp_pareto_merged.json" --inputs "$c0" "$c1"
fi

echo "[cowp_v7] complete: $OUT_ROOT"
