...
# Paper-grade data rebuild for v16.7.  Uses a fresh root because priority labels,
...
COWP_ROOT="${COWP_ROOT:-/data0/senzeyu2/dataset/COWP/formal_v17}"
...
LABEL_CFG="configs/label_cowp_v16.yaml"
EVAL_CFG="configs/eval_cowp_v16.yaml"
...
TRANSPORT_TRAIN="$COWP_ROOT/tensor_cache_train_waymax_transport_v17"
TRANSPORT_VAL="$COWP_ROOT/tensor_cache_val_waymax_transport_v17"
REPLAY_DIR="$COWP_ROOT/waymax_replay_v17"
...
run augment_train "$PYTHON_BIN" -u -m cowp.scripts.26_augment_transport_labels \
  ...
  --sidecar-subdir .transport_v17 --force
...
cat > "$COWP_ROOT/data_manifest_v17.json" <<JSON
{
  "schema_version": "cowp_v17_data_v1",
  ...
  "notes": "Fresh labels include corrected priority arrival order and map-screened kinematic proposal bank."
}
JSON

echo "[v16.7 data] complete: $COWP_ROOT"
