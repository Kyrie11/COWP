...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v5}"
...
if [[ "$DETACH" == "1" && "${COWP_V5_DETACHED:-0}" != "1" ]]; then
  export COWP_V5_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v5] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi

LABEL_RUNTIME="$OUT_ROOT/configs/label_cowp_v5.yaml"
TRAIN_RUNTIME="$OUT_ROOT/configs/train_cowp_v5.yaml"
...
p.update({
    ...
    'candidate_frontier_max_action_risk': 0.45,
    'candidate_frontier_shield_tie_mix': 0.08,
    'candidate_certificate_penalty': 3.0,
    'candidate_pair_risk_mix': 0.20,
    'candidate_pressure_prior_mix': 0.35,
    'candidate_rule_risk_mix': 1.0,
    'candidate_action_risk_mix': 1.5,
    'candidate_outcome_risk_mix': 1.0,
    'candidate_selection_risk_budget': 0.18,
    'candidate_selection_min_keep': 1,
    'candidate_selection_score_weight': 0.18,
    'candidate_selection_progress_weight': 0.10,
    'candidate_selection_action_weight': 1.15,
    'candidate_selection_rule_weight': 0.25,
    'candidate_selection_outcome_weight': 0.45,
...
w.update({
    ...
    # Mildly emphasize false-safe separation without turning the head into a
    # general collision predictor.  This supports the paper's coercion notion.
    'candidate_cert_pair_margin': 1.2,
})
...
echo "[cowp_v5] done. Results under $OUT_ROOT"
