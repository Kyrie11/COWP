...
OUT_ROOT="${OUT_ROOT:-outputs/v16_8_43_blocker_conditioned_interaction_aware_reachable_response_envelope}"
...
run_profile8_parallel2() {
  ...
  local tag="runtime_profile8_v43"
...
case "$MODE" in
  sanity)
    python -m py_compile cowp/models/cowp_model.py cowp/waymax_eval/policy_wrapper.py cowp/waymax_eval/rollout.py cowp/waymax_eval/metrics_cowp.py \
      ...
      cowp/scripts/92_analyze_conflict_window_control_reachable_tube.py cowp/scripts/93_analyze_shift_closed_first_action_viability_interval.py cowp/scripts/94_analyze_shift_closure_semantic_fidelity_repair.py cowp/scripts/96_analyze_blocker_conditioned_interaction_aware_reachable_response_envelope.py
    python -m pytest -q \
    ...
    ensure_ids
    echo "v16.8.43 BC-IARE semantic/integrity sanity passed"
    ;;
  make_ids) ensure_ids ;;
  build_tfindex)
  ...
  counterfactual48_parallel2)
    ensure_ids
    run_parallel_methods "counterfactual48_v43" "$CF48_IDS" "${METHODS:-cowp_blocker_conditioned_interaction_aware_reachable_response_envelope}" ;;
  analyze_counterfactual48)
    analyze_set "counterfactual48" "counterfactual48_v43" "$REF_CF48_COWP" "$REF_CF48_RVR" ;;
  fresh37_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_blocker_conditioned_interaction_aware_reachable_response_envelope}"
    require_gate_pass "$OUT_ROOT/counterfactual48_v43_blocker_conditioned_interaction_aware_reachable_response_envelope_analysis.json" "$promoted" "counterfactual48"
    run_parallel_methods "fresh37_v43" "$FRESH37_IDS" "$promoted" ;;
  analyze_fresh37)
    analyze_set "fresh37" "fresh37_v43" "$REF_FRESH37_COWP" "$REF_FRESH37_RVR" ;;
  confirm200_parallel2)
    ensure_ids
    promoted="${PROMOTED_METHODS:-cowp_blocker_conditioned_interaction_aware_reachable_response_envelope}"
    require_gate_pass "$OUT_ROOT/fresh37_v43_blocker_conditioned_interaction_aware_reachable_response_envelope_analysis.json" "$promoted" "fresh37"
  ...
  help|*)
    cat <<EOF
V16.8.43 BC-IARE fail-closed run order
...
Optional performance-only (never promotion evidence):
