...
OUT_ROOT="${OUT_ROOT:-outputs/cowp_v16_cnob_dynamics_v9labels_seed2026}"
...
cp configs/model_cowp_v16.yaml "$OUT_ROOT/configs/model_cowp_v16.yaml"
cp configs/label_cowp_v16.yaml "$OUT_ROOT/configs/label_cowp_v16.yaml"
cp configs/label_cowp_v16_pareto_ablation.yaml "$OUT_ROOT/configs/label_cowp_v16_pareto_ablation.yaml"
cp configs/label_cowp_v16_pairmax_ablation.yaml "$OUT_ROOT/configs/label_cowp_v16_pairmax_ablation.yaml"
cp configs/train_cowp_v16.yaml "$OUT_ROOT/configs/train_cowp_v16.yaml"
cp configs/eval_cowp_v16.yaml "$OUT_ROOT/configs/eval_cowp_v16.yaml"
sed -i -E "0,/^  seed:/{s/^  seed:.*/  seed: ${TRAIN_SEED}/}" "$OUT_ROOT/configs/train_cowp_v16.yaml"
...
if [[ "$DETACH" == "1" && "${COWP_V16_DETACHED:-0}" != "1" ]]; then
  export COWP_V16_DETACHED=1
  nohup bash "$0" > "$OUT_ROOT/logs/driver.nohup.log" 2>&1 &
  echo "[cowp_v16] detached pid=$! log=$OUT_ROOT/logs/driver.nohup.log"
  exit 0
fi
...
logrun gate_natural_effectiveness "$PYTHON_BIN" -u -m cowp.scripts.40_gate_natural_effectiveness \
...
if [[ "$STOP_AFTER_STAGE" == "natural" ]]; then
  echo "[cowp_v16] stopped after natural gate: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "transport" ]]; then
  echo "[cowp_v16] stopped after transport: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "planner" ]]; then
  echo "[cowp_v16] stopped after planner: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "offline" ]]; then
  echo "[cowp_v16] stopped after learned-offline verification: $OUT_ROOT"
  exit 0
fi
...
if [[ "$STOP_AFTER_STAGE" == "probe" ]]; then
  echo "[cowp_v16] stopped after probe: $OUT_ROOT"
  exit 0
fi
...
echo "[cowp_v16] complete: $OUT_ROOT"
